...
$orinal_career_title = ($_GET['url']);
$get_career_id = getCAREER_DETAILS('', $orinal_career_title, 'get_career_id');
...
<head>
    ...
    <title>Hiring of Security Service Agency at SIDBI Hyderabad - Small Industries
...
<body class="main-wrapper-en">
    ...
    <div id="wrapper" data-aos-easing="ease-out-back" data-aos-duration="1000" data-aos-delay="0">
        ...
        ?>
        <span id="showCareerDocument"></span>
    </div>
    ...
    <script>
        $(document).ready(function() {
            show_CAREER_DETAIL_DOCUMNET('<?= $get_configured_lang; ?>', '<?= $get_career_id; ?>');
        });

        function show_CAREER_DETAIL_DOCUMNET(LANGUAGE, ID) {
            $.ajax({
                type: 'post',
                url: '<?= BASEPATH; ?>engine/ajax/__ajax_career_details_document.php?type=show_document',
                data: {
                    ID: ID,
                    LANGUAGE: LANGUAGE
                },
                success: function(response) {
                    $('#showCareerDocument').html(response);
                }
            });
        }
    </script>
