...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "vertical_id=$id";

  if (sqlACTIONS("UPDATE", "vertical", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'vertical.php?code=5'; </script>";
  }
}
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#verticalLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONvertical.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "vertical_leadership" //1
            },
            {
              "data": "vertical_name" //2
            },
            {
              "data": "vertical_contact" //3
            },
            {
              "data": "vertical_email" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="verticalstatus-' + row.modify + '" id="verticalstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="verticalstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="verticalstatus--' + row.modify + '" id="verticalstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="verticalstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="vertical.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__vertical.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(vertical_id, status) {
      var SELECTED_ID = vertical_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__vertical.php?type=changestatus&vertical_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#verticalname').parsley();
      var old_vertical_name = document.getElementById("encrypt_vertical_name").value;
      window.ParsleyValidator.addValidator('checkverticalname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_vertical_name.php',
            method: "POST",
            data: {
              vertical_name: value,
              old_vertical_name: old_vertical_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#verticalcode').parsley();
      var old_vertical_name = document.getElementById("encrypt_verticalcode").value;
      window.ParsleyValidator.addValidator('checkverticalcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_vertical_code.php',
            method: "POST",
            data: {
              vertical_code: value,
              old_vertical_code: old_vertical_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add vertical.', 'error');
    }
