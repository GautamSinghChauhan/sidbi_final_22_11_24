...
if ($save == "save" || $save_close == "save_close") :
	...
	$harmonised_date = $validation_globalclass->sanitize(ucwords($_REQUEST['harmonised_date']));

	$harmonised_url = $validation_globalclass->sanitize($_REQUEST['harmonised_url']);

	$harmonised_title = htmlentities($harmonised_title);
	$harmonised_date = htmlentities($harmonised_title_hindi);
	...
	if (sqlACTIONS("INSERT", "harmonised", $arrFields, $arrValues, '')) :

		$harmonised_ID = sqlINSERTID_LABEL();

		if ($choose_document_move_file) :
			//Insert Query
			$arrFields_translations = array('`harmonised_id`', '`hin_filename`', '`language_id`', '`title`', '`createdby`', '`status`');
			$arrValues_translations = array("$harmonised_ID", "$choose_document_file_name", "2", "$harmonised_title_hindi", "$logged_user_id", "$status");
		else :
			//Insert Query
			$arrFields_translations = array('`harmonised_id`', '`language_id`', '`title`', '`createdby`', '`status`');
			$arrValues_translations = array("$harmonised_ID", "2", "$harmonised_title_hindi", "$logged_user_id", "$status");
		endif;

		if (sqlACTIONS("INSERT", "harmonised_translations", $arrFields_translations, $arrValues_translations, '')) :
		...
		if ($save == "save") : ?>
			<script type="text/javascript">
				window.location = 'harmonised.php?route=add&code=1'
			</script>
		<?php else : ?>
			<script type="text/javascript">
				window.location = 'harmonised.php?code=1'
			</script>
<?php
		endif;
	else :
					...
					<form method="post" enctype="multipart/form-data" data-parsley-validate>
						...
						<div id="basic">
							...
							<div class="form-group row">
								<label for="harmonised_status" class="col-sm-2 col-form-label"><?= $__active ?></label>
								<div class="col-sm-7">
									<div class="col-form-label custom-control custom-switch">
										<input type="checkbox" class="custom-control-input" name="harmonised_status" id="harmonised_status" checked="">
							...
							<div class="form-group row align-items-end">
								<div class="col-md-2">
									<label for="harmonised_title" class="form-label">Harmonised Title(EN)<span class="tx-danger">*</span></label>
								</div>

								<div class="col-md-5">
									<label for="overview_title" class="form-label">In English<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="harmonised_title" id="harmonised_title" required autocomplete="off" />
								</div>
							</div>

							<div class="form-group row align-items-end">
								<div class="col-md-2">
									<label for="harmonised_title" class="form-label">Harmonised Title(HI)<span class="tx-danger">*</span></label>
								</div>
							...
							<div class="form-group row">
								<div class="col-md-2">
									<label for="harmonised_date" class="form-label">Harmonised Date<span class="tx-danger">*</span></label>
								</div>
