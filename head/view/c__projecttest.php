...
if ($save == "save" || $save_close == "save_close") :
	...
	$projecttest_link = $validation_globalclass->sanitize(ucwords($_REQUEST['projecttest_link']));

	$projecttest_image = $validation_globalclass->sanitize($_REQUEST['projecttest_image']);

	// $projecttest_title = htmlentities($projecttest_title);
	// $projecttest_title_hindi = htmlentities($projecttest_title_hindi);
	// $projecttest_content = htmlentities($projecttest_content);
	// $projecttest_content_hindi = htmlentities($projecttest_content_hindi);

	
	$fileName =  $_FILES["projecttest_image"]["name"];
	...
	if (!empty($fileName)) :
		...
		if (in_array($ext, $valid_formats)) :
			...
			if ($file_size < 2097152) :
				...
				$new_file_projecttest_image = time() . substr(str_replace(" ", "_", $txt), 5) . "." . $ext;

				$new_file_name_stu  = pathinfo($new_file_projecttest_image, PATHINFO_EXTENSION);;
				$current_time = date("Ymdhis");
				$NEWfileName = 'sidbiprojecttest' . '_' . '.' . $new_file_name_stu;
				// echo $NEWfileName;
				// exit();
				// $upNEWfileNameloadStatus = 1;
				$files_upload = $_FILES['projecttest_image']["name"];
				...
				$projecttest_image = $NEWfileName;
				$fileType = pathinfo($targetFilePath, PATHINFO_EXTENSION);

				// Upload file to the server 
				if (move_uploaded_file($_FILES["projecttest_image"]["tmp_name"], $targetFilePath)) :
					$uploadedFile = $NEWfileName;
					unlink($uploadDir . $projecttest_image_old);
				endif;
			else :
				echo '3';
		...
		$projecttest_image = $projecttest_image_old;
	endif;
	...
	$arrValues = array("$projecttest_title", "$projecttest_title_hindi", "$projecttest_content", "$projecttest_content_hindi", "$projecttest_image", "$projecttest_link", "$logged_user_id", "$status");

	// print_r($arrFields);
	// print_r($arrValues);
	// exit;
	
	if (sqlACTIONS("INSERT", "projecttest", $arrFields, $arrValues, '')) :
		...
		// if (sqlACTIONS("INSERT", "projecttest_translaction", $arrFields_translations, $arrValues_translations, '')) :
		// endif;
		...
		if ($save == "save") : ?>
			<script type="text/javascript">
				window.location = 'projecttest.php?route=add&code=1'
			</script>
		<?php else : ?>
			<script type="text/javascript">
	           window.location = 'projecttest.php?code=1'
			</script>
<?php
		endif;
		exit();
					...
					<form method="post" enctype="multipart/form-data" data-parsley-validate>
						...
						<div id="basic">
							...
							<div class="form-group row align-items-end">
								...
								<div class="col-md-5">
									...
									<input type="text" class="form-control" name="projecttest_title" id="projecttest_title" autocomplete="off" />
								</div>
								<div class="col-md-5">
									<label for="projecttest_title_hindi" class="form-label">In Hindi<span class="tx-danger"></label>
									<input type="text" class="form-control" name="projecttest_title_hindi" id="projecttest_title_hindi" autocomplete="off" />
								</div>
							</div>
							...
							<div class="form-group row align-items-end">
								...
								<div class="col-md-5">
									...
									<textarea type="text" class="form-control" name="projecttest_content" id="projecttest_content" required autocomplete="off"></textarea>
								</div>
								<div class="col-md-5">
									<label for="projecttest_content_hindi" class="form-label">In Hindi<span class="tx-danger">*</span></label>
									<textarea type="text" class="form-control" name="projecttest_content_hindi" id="projecttest_content_hindi" required autocomplete="off"></textarea>
								</div>
						...
						<div class="form-group row">
							...
							<label for="projecttest_image" class="control-label">Image<span class="tx-danger"> *</span></label>
								<input class="form-control form-control-sm" name="projecttest_image" id="projecttest_image" type="file" value="<?php echo $projecttest_image ?>" autocomplete="off">
								<small class="text-danger">Allowed format JPG, JPEG, PNG, PDF, DOCX. Max file size 2 MB.</small>
								<input type="hidden" class="form-control" name="projecttest_image_old" id="projecttest_image_old" placeholder="Tutor Attachement" value="<?= $projecttest_image ?>" autocomplete="off">
								<span class="mg-l-10 badge badge-sm"><a class="text-white" target="_blank" href="<?= BASEPATH . 'assets/front/home/' . $projecttest_image; ?>"></a></span>
							</div>
							<div class="col-md-5">
									<label for="projecttest_link" class="form-label">Link<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="projecttest_link" id="projecttest_link" required autocomplete="off"/>
								</div>
