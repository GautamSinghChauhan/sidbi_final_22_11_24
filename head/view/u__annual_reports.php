...
if ($save == "update" && $id != '') :
	...
	$annual_reports_date = $validation_globalclass->sanitize($_REQUEST['annual_reports_date']);
	$annual_reports_date = dateformat_database($annual_reports_date);
	...
	if (sqlACTIONS("UPDATE", "annual_reports", $arrFields, $arrValues, $sqlWhere)) :
		...
		if ($choose_document_move_file) :
			...
			$arrValues_translations = array("$id", "$choose_document_file_name", "2", "$annual_reports_title_hindi", "$logged_user_id", "$status");
		else :
			//Insert Query
			$arrFields_translations = array('`annual_reports_id`', '`language_id`', '`title`', '`createdby`', '`status`');
			$arrValues_translations = array("$id", "2", "$annual_reports_title_hindi", "$logged_user_id", "$status");
		endif;

        $sqlWhere_translations = " `annual_reports_id` = '$id' ";
	
		if (sqlACTIONS("UPDATE", "annual_reports_translations", $arrFields_translations, $arrValues_translations, $sqlWhere_translations)) :
			echo "<script type='text/javascript'>window.location = 'annual_reports.php?code=1'</script>";
		endif;
...
if ($route == 'edit') :
	...
	$list_datas = sqlQUERY_LABEL("SELECT `annual_reports_id`, `annual_reports_title`, `filename`, `sort_order`, `user_id`, `createdby`, `createdon`, `updatedon`, `deleted`, `status` FROM `annual_reports` WHERE `deleted` = '0'  AND `annual_reports_id` = '$id'") or die("Unable to get records: " . sqlERROR_LABEL());
                    ...
                    <form method="post" enctype="multipart/form-data" data-parsley-validate>
                        ...
                        <div id="basic">
                            ...
                            <div class="form-group row">
                                <label for="annual_reports_status"
                                    class="col-sm-2 col-form-label"><?= $__active ?></label>
                                <div class="col-sm-7">
                                    <div class="col-form-label custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input" name="annual_reports_status"
                                            id="annual_reports_status" checked="">
                            ...
                            <div class="form-group row align-items-end">
                                <div class="col-md-2">
                                    <label for="annual_reports_title" class="form-label">Annual Reports Title<span
                                ...
                                <div class="col-md-5">
                                    <label for="overview_title" class="form-label">In English<span
                                            class="tx-danger">*</span></label>
                                    <input type="text" class="form-control" name="annual_reports_title"
                                        id="annual_reports_title" value="<?= $annual_reports_title ; ?>" required
                                        autocomplete="off" />
                                </div>
                            </div>
                            ...
                            <div class="form-group row align-items-end">
                                ...
                                <div class="col-md-5">
                                    ...
                                    <input type="text" class="form-control" name="title" id="title"
                                        value="<?= $annual_reports_title ; ?>" required autocomplete="off" />
                                </div>
                            </div>
