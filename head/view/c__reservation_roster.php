...
if ($save == "save" || $save_close == "save_close") :
	...
	if ($choose_document_move_file) :
		...
		//Insert Query
		$arrFields = array('`reservation_roster_title`', '`createdby`', '`status`');
		$arrValues = array("$reservation_roster_title", "$logged_user_id", "$status");
	endif;

	if (sqlACTIONS("INSERT", "reservation_roster", $arrFields, $arrValues, '')) :

		$reservation_roster_ID = sqlINSERTID_LABEL();

		if ($choose_document_move_file) :
			//Insert Query
			$arrFields_translations = array('`reservation_roster_id`', '`hin_filename`', '`language_id`', '`title`', '`createdby`', '`status`');
			$arrValues_translations = array("$reservation_roster_ID", "$choose_document_file_name", "2", "$reservation_roster_title_hindi", "$logged_user_id", "$status");
		else :
			//Insert Query
			$arrFields_translations = array('`reservation_roster_id`', '`language_id`', '`title`', '`createdby`', '`status`');
			$arrValues_translations = array("$reservation_roster_ID", "2", "$reservation_roster_title_hindi", "$logged_user_id", "$status");
		endif;

		if (sqlACTIONS("INSERT", "reservation_roster_translations", $arrFields_translations, $arrValues_translations, '')) :
					...
					<form method="post" enctype="multipart/form-data" data-parsley-validate>
						...
						<div id="basic">
							...
							<div class="form-group row align-items-end">
								<div class="col-md-2">
									<label for="reservation_roster_title" class="form-label">Reservation Roaster Title (EN)<span class="tx-danger">*</span></label>
								</div>

								<div class="col-md-5">
									<label for="overview_title" class="form-label">In English<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="reservation_roster_title" id="reservation_roster_title" required autocomplete="off" />
								</div>
							</div>

							<div class="form-group row align-items-end">
								<div class="col-md-2">
									<label for="reservation_roster_title" class="form-label">Reservation Roaster Title (HI)<span class="tx-danger">*</span></label>
								</div>
