...
if ($save == "save" || $save_close == "save_close") :
	...
	// $financialreport_last_date = $validation_globalclass->sanitize($_REQUEST['financialreport_last_date']);
	// $financialreport_last_date = dateformat_database($financialreport_last_date);

	//Insert Query
	$arrFields = array('`financial_report_title`', '`report_date`', '`documents`', '`createdby`', '`status`');

	$arrValues = array("$financial_report_title", "$report_date", "$documents", "$logged_user_id", "$status");

	if (sqlACTIONS("INSERT", "financial_reports", $arrFields, $arrValues, '')) :

		$financial_report_id  = sqlINSERTID_LABEL();

		//Insert Query
		$arrFields_translations = array('`financial_report_id`', '`language_id`', '`title`', '`hin_documents`', '`createdby`', '`status`');

		$arrValues_translations = array("$financial_report_id", "2", "$title", "$hin_documents", "$logged_user_id", "$status");

		if (sqlACTIONS("INSERT", "financial_report_translations", $arrFields_translations, $arrValues_translations, '')) :
		endif;
			...
			if ($document_count > 0) :
				for ($i = 1; $i <= $document_count; $i++) :
					if (isset($_FILES['document']['name'][$i]) && $_FILES['document']['name'][$i] != '') :
						$upload_dir = 'uploads/financialreport_document/';
						...
						if ($move_file) :
							$arrFields_gallery = array('`financial_report_id`', '`financialreport_document_language_id`', '`financialreport_document_type`', '`financialreport_document_name`', '`createdby`', '`status`');

							$arrValues_gallery = array("$financial_report_id", "$financialreport_document_language_id", "$financialreport_document_type", "$file_name", "$logged_user_id", "1");

							// print_r($arrFields_gallery );
							// print_r($arrValues_gallery);
							// exit;

							if (sqlACTIONS("INSERT", "js_financial_documents", $arrFields_gallery, $arrValues_gallery, '')) :
		...
		if ($save == "save") : ?>
			<script type="text/javascript">
				window.location = 'financialreport.php?route=add&code=1'
			</script>
		<?php else : ?>
			<script type="text/javascript">
				window.location = 'financialreport.php?code=1'
			</script>
<?php
		endif;
		exit();
					...
					<form method="post" enctype="multipart/form-data" data-parsley-validate>
						...
						<div id="basic">
							...
							<div class="form-group row align-items-end">
								<div class="col-md-2">
									<label for="financial_report_title" class="form-label">Financial Report Title<span class="tx-danger">*</span></label>
								</div>

								<div class="col-md-5">
									<label for="overview_title" class="form-label">In English<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="financial_report_title" id="financial_report_title" required autocomplete="off" />
								</div>
								<div class="col-md-5">
