...
if ($save == "save" || $save_close == "save_close") :
	...
	$workingtest_link = $validation_globalclass->sanitize(ucwords($_REQUEST['workingtest_link']));

	$workingtest_image = $validation_globalclass->sanitize($_REQUEST['workingtest_image']);

	// $workingtest_title = htmlentities($workingtest_title);
	// $workingtest_title_hindi = htmlentities($workingtest_title_hindi);
	// $workingtest_content = htmlentities($workingtest_content);
	// $workingtest_content_hindi = htmlentities($workingtest_content_hindi);

	
	$fileName =  $_FILES["workingtest_image"]["name"];
	...
	if (!empty($fileName)) :
		...
		if (in_array($ext, $valid_formats)) :
			...
			if ($file_size < 2097152) :
				...
				$new_file_workingtest_image = time() . substr(str_replace(" ", "_", $txt), 5) . "." . $ext;

				$new_file_name_stu  = pathinfo($new_file_workingtest_image, PATHINFO_EXTENSION);;
				$current_time = date("Ymdhis");
				$NEWfileName = 'sidbiworkingtest' . '_' . '.' . $new_file_name_stu;
				// echo $NEWfileName;
				// exit();
				// $upNEWfileNameloadStatus = 1;
				$files_upload = $_FILES['workingtest_image']["name"];
				...
				$workingtest_image = $NEWfileName;
				$fileType = pathinfo($targetFilePath, PATHINFO_EXTENSION);

				// Upload file to the server 
				if (move_uploaded_file($_FILES["workingtest_image"]["tmp_name"], $targetFilePath)) :
					$uploadedFile = $NEWfileName;
					unlink($uploadDir . $workingtest_image_old);
				endif;
			else :
				echo '3';
		...
		$workingtest_image = $workingtest_image_old;
	endif;
	...
	$arrValues = array("$workingtest_title", "$workingtest_title_hindi", "$workingtest_content", "$workingtest_content_hindi", "$workingtest_image", "$workingtest_link", "$logged_user_id", "$status");

	// print_r($arrFields);
	// print_r($arrValues);
	// exit;
	
	if (sqlACTIONS("INSERT", "workingtest", $arrFields, $arrValues, '')) :
		...
		// if (sqlACTIONS("INSERT", "workingtest_translaction", $arrFields_translations, $arrValues_translations, '')) :
		// endif;
		...
		if ($save == "save") : ?>
			<script type="text/javascript">
				window.location = 'workingtest.php?route=add&code=1'
			</script>
		<?php else : ?>
			<script type="text/javascript">
	           window.location = 'workingtest.php?code=1'
			</script>
<?php
		endif;
		exit();
					...
					<form method="post" enctype="multipart/form-data" data-parsley-validate>
						...
						<div id="basic">
							...
							<div class="form-group row align-items-end">
								...
								<div class="col-md-5">
									...
									<input type="text" class="form-control" name="workingtest_title" id="workingtest_title" autocomplete="off" />
								</div>
								<div class="col-md-5">
									<label for="workingtest_title_hindi" class="form-label">In Hindi<span class="tx-danger"></label>
									<input type="text" class="form-control" name="workingtest_title_hindi" id="workingtest_title_hindi" autocomplete="off" />
								</div>
							</div>
							...
							<div class="form-group row align-items-end">
								...
								<div class="col-md-5">
									...
									<textarea type="text" class="form-control" name="workingtest_content" id="workingtest_content" required autocomplete="off"></textarea>
								</div>
								<div class="col-md-5">
									<label for="workingtest_content_hindi" class="form-label">In Hindi<span class="tx-danger">*</span></label>
									<textarea type="text" class="form-control" name="workingtest_content_hindi" id="workingtest_content_hindi" required autocomplete="off"></textarea>
								</div>
						...
						<div class="form-group row">
							...
							<label for="workingtest_image" class="control-label">Image<span class="tx-danger"> *</span></label>
								<input class="form-control form-control-sm" name="workingtest_image" id="workingtest_image" type="file" value="<?php echo $workingtest_image ?>" autocomplete="off">
								<small class="text-danger">Allowed format JPG, JPEG, PNG, PDF, DOCX. Max file size 2 MB.</small>
								<input type="hidden" class="form-control" name="workingtest_image_old" id="workingtest_image_old" placeholder="Tutor Attachement" value="<?= $workingtest_image ?>" autocomplete="off">
								<span class="mg-l-10 badge badge-sm"><a class="text-white" target="_blank" href="<?= BASEPATH . 'assets/front/home/' . $workingtest_image; ?>"></a></span>
							</div>
							<div class="col-md-5">
									<label for="workingtest_link" class="form-label">Link<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="workingtest_link" id="workingtest_link" required autocomplete="off"/>
								</div>
