...
if (isset($update) && $update == "update" && $hidden_msme_pluse_id != '') {
    ...
    $sqlWhere = " `msme_pluse_id` = '$hidden_msme_pluse_id' ";

    if (sqlACTIONS("UPDATE", "js_msmepulse", $arrFields, $arrValues, $sqlWhere)) {
...
if ($route == 'edit') :
    ...
    $list_datas = sqlQUERY_LABEL("SELECT `msme_pluse_id`, `msme_title`, `msme_hindi_title`, `msme_pluse_description`, `msme_pluse_hindi_description`, `status` FROM `js_msmepulse` WHERE `deleted` = '0'  AND `msme_pluse_id` = '$id'") or die("Unable to get records: " . sqlERROR_LABEL());
    $check_record_availabity = sqlNUMOFROW_LABEL($list_datas);

    if ($check_record_availabity > 0) :
        while ($row = sqlFETCHARRAY_LABEL($list_datas)) :
            $msme_pluse_id = $row["msme_pluse_id"];
            $msme_title = html_entity_decode(html_entity_decode($row['msme_title']));
            $msme_hindi_title = html_entity_decode($row['msme_hindi_title']);
                ...
                <form method="post" enctype="multipart/form-data" data-parsley-validate>
                    ...
                    <div id="basic">
                        ...
                        <div class="form-group row">
                            ...
                            <input type="hidden" name="hidden_msme_pluse_id" value="<?= $id; ?>" />
                            <div class="col-sm-7">
                        ...
                        <div class="form-group row">
                                ...
                                <div class="col-md-5">
                                    ...
                                    <textarea type="text" class="form-control" name="msme_description" id="msme_description" placeholder=""><?= $msme_pluse_description; ?></textarea>
                                </div>
                                <div class="col-md-5">
                                    <label for="subsidiary_description" class="form-label">In Hindi<span class="tx-danger"></span></label>
                                    <textarea type="text" class="form-control" name="msme_description_hindi" id="msme_description_hindi" placeholder=""><?= $msme_pluse_hindi_description; ?></textarea>
                                </div>
                            </div>
                        <!-- <div class="form-group row align-items-end">
                            <div class="col-md-2">
