...
//Update Operation
if ($save == "update" && $hidden_career_ID != '') :
	...
	$sqlWhere = " career_id = $hidden_career_ID ";

	if (sqlACTIONS("UPDATE", "careers", $arrFields, $arrValues, $sqlWhere)) :

		//Update Query
		$arrFields_translations = array('`career_id`', '`language_id`', '`title`', '`createdby`', '`status`');

		$arrValues_translations = array("$hidden_career_ID", "2", "$career_title_hindi", "$logged_user_id", "$status");

		$sqlWhere_translations = " career_id = $hidden_career_ID ";

		if (sqlACTIONS("UPDATE", "career_translations", $arrFields_translations, $arrValues_translations, $sqlWhere_translations)) :
		endif;

		$arrayCount = count(array($_FILES));
			...
			if ($document_count > 0) :
				for ($i = 1; $i <= $document_count; $i++) :
					if (isset($_FILES['document']['name'][$i]) && $_FILES['document']['name'][$i] != '') :
						...
						if ($move_file) :
							$arrFields_gallery = array('`career_id`', '`career_document_language_id`', '`career_document_language_id`', '`career_document_type`', '`career_document_name`', '`createdby`', '`status`');
...
?>
		<script type="text/javascript">
			//window.location = 'career.php?code=2'
		</script>
<?php
...
if ($route == 'edit' && $id != '') {
	...
	$list_datas = sqlQUERY_LABEL("SELECT careers.new_career, careers.career_id, career_translations.career_id, careers.career_title, career_translations.title, careers.career_start_date, careers.career_expiry_date, careers.status,careers.career_url FROM careers INNER JOIN career_translations ON careers.career_id = career_translations.career_id where careers.deleted = '0' and careers.`career_id`='$id'") or die("Unable to get records:" . sqlERROR_LABEL());
					...
					<form method="post" enctype="multipart/form-data" data-parsley-validate>
						...
						<div id="stickThis" class="form-group row mg-b-0">
							...
							<div class="col-9 col-sm-6 text-right">
								...
								<input type="hidden" name="hidden_career_ID" value="<?= $career_id; ?>" />
							</div>
						</div>
						...
						<div id="basic">
							...
							<div class="form-group row">
								...
								<div class="col-sm-7">
									<div class="col-form-label custom-control custom-switch">
										<input type="checkbox" class="custom-control-input" name="career_status" id="career_status" <?php if ($status == 1) : echo 'checked';
																																	endif; ?>>
							...
							<div class="form-group row">
								...
								<div class="col-sm-7">
									<div class="col-form-label custom-control custom-switch">
										<input type="checkbox" class="custom-control-input" name="career_new_gif" id="career_new_gif" <?php if ($new_career == 1) : echo 'checked';
																																		endif; ?>>
							...
							<div class="form-group row align-items-end">
								...
								<div class="col-md-5">
									...
									<input type="text" class="form-control" name="career_title" id="career_title" value="<?= $career_title; ?>" required autocomplete="off" />
								</div>
								<div class="col-md-5">
									<label for="title" class="form-label">In Hindi<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="title" id="title" value="<?= $career_title_hindi; ?>" required autocomplete="off" />
								</div>
							</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-5">
									<input type="text" class="form-control" placeholder="DD/MM/YYYY" name="career_start_date" id="career_start_date" value="<?= $career_start_date; ?>" required autocomplete="off" />
								</div>
							</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-5">
									<input type="text" class="form-control" placeholder="DD/MM/YYYY" name="career_expiry_date" id="career_expiry_date" value="<?= $career_expiry_date; ?>" required autocomplete="off" />
								</div>
							</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-5">
									<input type="text" class="form-control" placeholder="career URL" name="career_url" id="career_url" value="<?= $career_url; ?>" required autocomplete="off" />
								</div>
							</div>
						...
						<div id="document">
							...
							<div class="col-md-12">
								...
								<div class="row">
									<?php
									$list_document_datas = sqlQUERY_LABEL("SELECT `career_document_id`, `career_id`, `career_document_language_id`, `career_document_type`, `career_document_name` FROM `js_career_documents` WHERE deleted = '0' and `career_id`='$id'") or die("Unable to get records:" . sqlERROR_LABEL());
												...
												<div id="uploadedFilesArea" class="mt-3">
													<div class="row" id="uploadedFileList">
														...
														while ($row_document = sqlFETCHARRAY_LABEL($list_document_datas)) :
															...
															$career_id = $row_document["career_id"];
															$career_document_language_id = $row_document["career_document_language_id"];
															$career_document_type = $row_document["career_document_type"];
																...
																<div class="p-2 rounded position-relative alert alert-primary">
																	...
																	<div class="text-center">
																		<div class="vendor-vehicle-image-container">
																			<iframe src="uploads/career_document/<?= $career_document_name; ?>" width="600" height="400" frameborder="0" class="d-block w-100 h-100 rounded"></iframe>
																			<div class="vendor-vehicle-download-button" onclick="downloadImage('uploads/career_document/<?= $career_document_name; ?>')"><i class="ti ti-download ti-sm"></i></div>
																			<input type="text" name="career_document_type_id[<?= $counter; ?>]" id="career_document_type_id" value="<?= $career_document_type; ?>" hidden="" />
																			<input type="text" name="career_document_language_id[<?= $counter; ?>]" id="career_document_language_id" value="<?= $career_document_language_id; ?>" hidden="" />
																			<input type="file" name="document[<?= $counter; ?>]" id="uploadDocument_<?= $counter; ?>" hidden="" />
																		</div>
																	</div>
																	<div class="text-center mt-2">
																		<p class="card-text mb-0 text-center text-break"><?= $career_document_name; ?></p>
																	</div>
																</div>
															</div>
														<?php endwhile; ?>
