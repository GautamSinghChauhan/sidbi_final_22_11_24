...
// if ($update == "update" && $id != '') :
	...
	$financialreport_status = $validation_globalclass->sanitize($_REQUEST['financialreport_status']);

	if ($financialreport_status == 'on') :
		$status = '1';
	else :
		$status = '0';
	endif;
	
	$financial_report_title = $validation_globalclass->sanitize(ucwords($_REQUEST['financial_report_title']));
	$report_date = $validation_globalclass->sanitize($_REQUEST['report_date']);
	$report_date = dateformat_database($report_date);
	$documents = $validation_globalclass->sanitize(ucwords($_REQUEST['documents']));
	$title = $validation_globalclass->sanitize(ucwords($_REQUEST['title']));
	$hin_documents = $validation_globalclass->sanitize(ucwords($_REQUEST['hin_documents']));

	$financialreport_url = $validation_globalclass->sanitize($_REQUEST['financialreport_url']);

	$financial_report_title = htmlentities($financial_report_title);
	$documents = htmlentities($documents);
	$title = htmlentities($title);
	$hin_documents = htmlentities($hin_documents);

	//Update Query
	$arrFields = array('`financial_report_title`', '`report_date`', '`createdby`', '`status`');

	$arrValues = array("$financial_report_title", "$report_date", "$logged_user_id", "$status");

	$sqlWhere = " financial_report_id  = $id ";

	if (sqlACTIONS("UPDATE", "financial_reports", $arrFields, $arrValues, $sqlWhere)) :


		$financial_report_ID = $id;

		//Update Query
		$arrFields_translations = array('`financial_report_id`', '`language_id`', '`title`', '`createdby`', '`status`');

		$arrValues_translations = array("$id", "2", "$title", "$logged_user_id", "$status");

		$sqlWhere_translations = " financial_report_id = $id ";

		if (sqlACTIONS("UPDATE", "financial_report_translations", $arrFields_translations, $arrValues_translations, $sqlWhere_translations)) :
		endif;
			...
			if ($document_count > 0) :
				for ($i = 1; $i <= $document_count; $i++) :
					if (isset($_FILES['document']['name'][$i]) && $_FILES['document']['name'][$i] != '') :
						$upload_dir = 'uploads/financial_document/';
						$file_extension = strtolower(end(explode('.', $_FILES['document']['name'][$i])));
						$file_name = str_replace(' ', '_', $financialreport_title) . '-' . rand(0, 99999) . time() . '.' . $file_extension;
						...
						$financialreport_document_language_id = $_POST['financialreport_document_language_id'][$i];
						$financialreport_document_type = $_POST['financialreport_document_type_id'][$i];

						if ($move_file) :
							$arrFields_gallery = array('`financialreport_id`', '`financialreport_document_language_id`', '`financialreport_document_type`', '`financialreport_document_name`', '`createdby`', '`status`');
							$arrValues_gallery = array("$financialreport_ID", "$financialreport_document_language_id", "$financialreport_document_type", "$file_name", "$logged_user_id", "1");
							if (sqlACTIONS("INSERT", "js_financialreport_documents", $arrFields_gallery, $arrValues_gallery, '')) :
							endif;
						endif;
					endif;
				endfor;
...
?>
		<script type="text/javascript">
			window.location = 'financialreport.php?code=2'
		</script>
<?php
...
if ($route == 'edit' && $id != '') {
	...
	$list_datas = sqlQUERY_LABEL("SELECT financialreport.`financial_report_id`, financialreport.`financial_report_title`, financialreport_TRANSLATION.`title`, financialreport.`report_date`, financialreport.`status` FROM `financial_reports` AS financialreport INNER JOIN financial_report_translations AS financialreport_TRANSLATION ON financialreport.financial_report_id=financialreport_TRANSLATION.financial_report_id where financialreport.deleted = '0' and financialreport.`financial_report_id`='$id'") or die("Unable to get records:" . sqlERROR_LABEL());
	...
	while ($row = sqlFETCHARRAY_LABEL($list_datas)) {
		...
		$title = $row["title"];
		$report_date = dateformat_datepicker($row['report_date']);
		$status = $row["status"];
	}
}
?>
					...
					<form method="post" enctype="multipart/form-data" data-parsley-validate>
						...
						<div id="stickThis" class="form-group row mg-b-0">
							...
							<button type="submit" name="save" value="update"
								...
								<input type="hidden" name="hidden_financial_report_id" value="<?= $financial_report_id; ?>" />
							</div>
						</div>
						...
						<div id="basic">
							...
							<div class="form-group row">
								<label for="financialreport_status" class="col-sm-2 col-form-label"><?= $__active ?></label>
								<div class="col-sm-7">
									<div class="col-form-label custom-control custom-switch">
										<input type="checkbox" class="custom-control-input" name="financialreport_status" id="financialreport_status" checked="">
										<label class="custom-control-label" for="financialreport_status">Yes</label>
									</div>
								</div>
							</div>
							...
							<div class="form-group row align-items-end">
								<div class="col-md-2">
									<label for="financial_report_title" class="form-label">financialreport Title<span class="tx-danger">*</span></label>
								</div>

								<div class="col-md-5">
									<label for="overview_title" class="form-label">In English<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="financial_report_title" id="financial_report_title" required autocomplete="off" value="<?= $financial_report_title; ?>">
								</div>
								<div class="col-md-5">
									<label for="title" class="form-label">In Hindi<span class="tx-danger">*</span></label>
									<input type="text" class="form-control" name="title" id="title" required autocomplete="off" value="<?= $title; ?>" />
								</div>
							</div>
							...
							<div class="form-group row">
								<div class="col-md-2">
									<label for="report_date" class="form-label">financial Report Date<span class="tx-danger">*</span></label>
								</div>

								<div class="col-md-5">
									<input type="text" class="form-control" placeholder="DD/MM/YYYY" name="report_date" id="report_date" value="<?= $report_date; ?>" required autocomplete="off" />
								</div>
							</div>
						...
						<div id="document">
							...
							<div class="col-md-12">
								...
								<div class="row">
									<?php
									$list_document_datas = sqlQUERY_LABEL("SELECT `financial_report_document_id`, `financial_report_id`, `financialreport_document_language_id`, `financialreport_document_type`, `financialreport_document_name` FROM `js_financial_documents` WHERE deleted = '0' and `financial_report_id`='$id'") or die("Unable to get records:" . sqlERROR_LABEL());
												...
												<div id="uploadedFilesArea" class="mt-3">
													<div class="row" id="uploadedFileList">
														...
														while ($row_document = sqlFETCHARRAY_LABEL($list_document_datas)) :
															$counter++;
															$financial_report_document_id = $row_document["financial_report_document_id"];
															$financial_report_id = $row_document["financial_report_id"];
															$financialreport_document_language_id = $row_document["financialreport_document_language_id"];
															$financialreport_document_type = $row_document["financialreport_document_type"];
															$financialreport_document_name = $row_document["financialreport_document_name"];
														?>
																...
																<div class="p-2 rounded position-relative alert alert-primary">
																	...
																	<div class="text-center">
																		<div class="vendor-vehicle-image-container">
																			<iframe src="uploads/financialreport_document/<?= $financialreport_document_name; ?>" width="600" height="400" frameborder="0" class="d-block w-100 h-100 rounded"></iframe>
																			<div class="vendor-vehicle-download-button" onclick="downloadImage('uploads/financialreport_document/<?= $financialreport_document_name; ?>')"><i class="ti ti-download ti-sm"></i></div>
																			<input type="text" name="financialreport_document_type_id[<?= $counter; ?>]" id="financialreport_document_type_id" value="<?= $financialreport_document_type; ?>" hidden="" />
																			<input type="text" name="financialreport_document_language_id[<?= $counter; ?>]" id="financialreport_document_language_id" value="<?= $financialreport_document_language_id; ?>" hidden="" />
																			<input type="file" name="document[<?= $counter; ?>]" id="uploadDocument_<?= $counter; ?>" hidden="" />
																		</div>
																	</div>
																	<div class="text-center mt-2">
																		<p class="card-text mb-0 text-center text-break"><?= $financialreport_document_name; ?></p>
																	</div>
																</div>
															</div>
														<?php endwhile; ?>
							...
							<form id="ajax_upload_document_form" enctype="multipart/form-data" method="POST" data-parsley-validate>
								...
								<div class="row mt-2">
									<!-- <div class="col-12 mb-3">
										<label class="form-label" for="financialreport_document_type_id">Document Type<span class=" text-danger"> *</span></label>

										<select name="financialreport_document_type_id" id="financialreport_document_type_id" class="form-control" data-live-search="true" required>
									...
									<div class="col-12 mb-3">
										<label class="form-label" for="financialreport_document_language_id">Language<span class=" text-danger"> *</span></label>

										<select name="financialreport_document_language_id" id="financialreport_document_language_id" class="form-control" data-live-search="true" required>
