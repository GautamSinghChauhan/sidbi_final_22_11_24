...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "cipos_orders", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'cpios.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
    $(function() {
      ...
      $('#year').datepicker({
        autoclose: true,
        startView: 2, // Set the start view to show only years
        minViewMode: 2, // Display only years
        format: 'yyyy', // Set the format to show only the year
      }).attr('readonly', 'readonly');

      <?php if ($route == '') : ?>
        $('#ciposLIST').DataTable({
          // DataTables options here
          'ajax': 'engine/json/JSONcipos.php',
          "columns": [{
              "data": "count" //0
            },
            {
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="cpios.php?route=edit&id=' + data + '" class="btn btn-light btn-icon mr-1"><i class="fa fa-pencil-alt"></i> <a href="cpios.php?route=preview&id=' + data + '" title="Click to preview" class="btn btn-light btn-icon"><i class="fa fa-eye"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    <?php elseif ($route == 'preview') : ?>
      $('#ciposfyLIST').DataTable({
        // DataTables options here
        'ajax': 'engine/json/JSONciposfy.php',
        "columns": [{
            "data": "count" //0
          },
          {
            "data": "document_title_english" //1
          },
          {
            "data": "document_title_hindi" //2
          },
          {
            "data": "document_name" //3
          },
          {
            "data": "status" //4
          },
    ...
    function togglestatusITEM(cipos_id, status) {
      var SELECTED_ID = cipos_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__cpios.php?type=changestatus&cipos_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }

    function togglestatusDOC(cipos_doc_id, status) {
      var SELECTED_ID = cipos_doc_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__cpios.php?type=doc_status&cipos_doc_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__cpios.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
    ...
    function deleteDOC(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__cpios.php?type=doc_delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
