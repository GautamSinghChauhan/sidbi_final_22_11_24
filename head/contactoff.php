...
<body>
  ...
  <script>
     if ($("#contactoff_content,#contactoff_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#contactoff_content, textarea#contactoff_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#contactoffimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__contactoff.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(contactoff_ID, status) {
      var SELECTED_ID = contactoff_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__contactoff.php?type=changestatus&contactoff_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#contactoffname').parsley();
      var old_contactoff_name = document.getElementById("encrypt_contactoff_name").value;
      window.ParsleyValidator.addValidator('checkcontactoffname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_contactoff_name.php',
            method: "POST",
            data: {
              contactoff_name: value,
              old_contactoff_name: old_contactoff_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#contactoffcode').parsley();
      var old_contactoff_name = document.getElementById("encrypt_contactoffcode").value;
      window.ParsleyValidator.addValidator('checkcontactoffcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_contactoff_code.php',
            method: "POST",
            data: {
              contactoff_code: value,
              old_contactoff_code: old_contactoff_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add contactoff.', 'error');
    }
