...
<body>
  ...
  <script>
     if ($("#policyadvocacy_content,#policyadvocacy_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#policyadvocacy_content, textarea#policyadvocacy_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#policyadvocacyimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__policyadvocacy.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(policyadvocacy_ID, status) {
      var SELECTED_ID = policyadvocacy_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__policyadvocacy.php?type=changestatus&policyadvocacy_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#policyadvocacyname').parsley();
      var old_policyadvocacy_name = document.getElementById("encrypt_policyadvocacy_name").value;
      window.ParsleyValidator.addValidator('checkpolicyadvocacyname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_policyadvocacy_name.php',
            method: "POST",
            data: {
              policyadvocacy_name: value,
              old_policyadvocacy_name: old_policyadvocacy_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#policyadvocacycode').parsley();
      var old_policyadvocacy_name = document.getElementById("encrypt_policyadvocacycode").value;
      window.ParsleyValidator.addValidator('checkpolicyadvocacycode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_policyadvocacy_code.php',
            method: "POST",
            data: {
              policyadvocacy_code: value,
              old_policyadvocacy_code: old_policyadvocacy_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add policyadvocacy.', 'error');
    }
