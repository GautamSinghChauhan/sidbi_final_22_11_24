...
<body>
  ...
  <script>
     if ($("#fixedscheme_content,#fixedscheme_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#fixedscheme_content, textarea#fixedscheme_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#fixedschemeimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__fixedscheme.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(fixedscheme_ID, status) {
      var SELECTED_ID = fixedscheme_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__fixedscheme.php?type=changestatus&fixedscheme_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#fixedschemename').parsley();
      var old_fixedscheme_name = document.getElementById("encrypt_fixedscheme_name").value;
      window.ParsleyValidator.addValidator('checkfixedschemename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_fixedscheme_name.php',
            method: "POST",
            data: {
              fixedscheme_name: value,
              old_fixedscheme_name: old_fixedscheme_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#fixedschemecode').parsley();
      var old_fixedscheme_name = document.getElementById("encrypt_fixedschemecode").value;
      window.ParsleyValidator.addValidator('checkfixedschemecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_fixedscheme_code.php',
            method: "POST",
            data: {
              fixedscheme_code: value,
              old_fixedscheme_code: old_fixedscheme_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add fixedscheme.', 'error');
    }
