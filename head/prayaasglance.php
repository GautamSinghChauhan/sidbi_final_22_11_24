...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "prayaasglance_id=$id";

  if (sqlACTIONS("UPDATE", "prayaasglance", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'prayaasglance.php?code=5'; </script>";
  }
}
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#prayaasglanceLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONprayaasglance.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "prayaasglance_heading" //1
            },
            {
              "data": "prayaasglance_title" //2
            },
            {
              "data": "prayaasglance_content" //3
            },
            {
              "data": "status" //4
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="prayaasglancestatus-' + row.modify + '" id="prayaasglancestatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="prayaasglancestatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="prayaasglancestatus--' + row.modify + '" id="prayaasglancestatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="prayaasglancestatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="prayaasglance.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
      ...
      if ($route == 'add' || $route == 'edit') {  ?>
        ...
        $("#prayaasglanceimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__prayaasglance.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(prayaasglance_id, status) {
      var SELECTED_ID = prayaasglance_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__prayaasglance.php?type=changestatus&prayaasglance_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#prayaasglancename').parsley();
      var old_prayaasglance_name = document.getElementById("encrypt_prayaasglance_name").value;
      window.ParsleyValidator.addValidator('checkprayaasglancename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_prayaasglance_name.php',
            method: "POST",
            data: {
              prayaasglance_name: value,
              old_prayaasglance_name: old_prayaasglance_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#prayaasglancecode').parsley();
      var old_prayaasglance_name = document.getElementById("encrypt_prayaasglancecode").value;
      window.ParsleyValidator.addValidator('checkprayaasglancecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_prayaasglance_code.php',
            method: "POST",
            data: {
              prayaasglance_code: value,
              old_prayaasglance_code: old_prayaasglance_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add prayaasglance.', 'error');
    }
