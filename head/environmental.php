...
<body>
  ...
  <script>
     if ($("#environmental_content,#environmental_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#environmental_content, textarea#environmental_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#environmentalimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__environmental.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(environmental_ID, status) {
      var SELECTED_ID = environmental_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__environmental.php?type=changestatus&environmental_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#environmentalname').parsley();
      var old_environmental_name = document.getElementById("encrypt_environmental_name").value;
      window.ParsleyValidator.addValidator('checkenvironmentalname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_environmental_name.php',
            method: "POST",
            data: {
              environmental_name: value,
              old_environmental_name: old_environmental_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#environmentalcode').parsley();
      var old_environmental_name = document.getElementById("encrypt_environmentalcode").value;
      window.ParsleyValidator.addValidator('checkenvironmentalcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_environmental_code.php',
            method: "POST",
            data: {
              environmental_code: value,
              old_environmental_code: old_environmental_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add environmental.', 'error');
    }
