...
<body>
  ...
  <script>
     if ($("#pilotfinance_content,#pilotfinance_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#pilotfinance_content, textarea#pilotfinance_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#pilotfinanceimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__pilotfinance.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(pilotfinance_ID, status) {
      var SELECTED_ID = pilotfinance_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__pilotfinance.php?type=changestatus&pilotfinance_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#pilotfinancename').parsley();
      var old_pilotfinance_name = document.getElementById("encrypt_pilotfinance_name").value;
      window.ParsleyValidator.addValidator('checkpilotfinancename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_pilotfinance_name.php',
            method: "POST",
            data: {
              pilotfinance_name: value,
              old_pilotfinance_name: old_pilotfinance_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#pilotfinancecode').parsley();
      var old_pilotfinance_name = document.getElementById("encrypt_pilotfinancecode").value;
      window.ParsleyValidator.addValidator('checkpilotfinancecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_pilotfinance_code.php',
            method: "POST",
            data: {
              pilotfinance_code: value,
              old_pilotfinance_code: old_pilotfinance_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add pilotfinance.', 'error');
    }
