...
<body>
  ...
  <script>
     if ($("#thirdparty_content,#thirdparty_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#thirdparty_content, textarea#thirdparty_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#thirdpartyimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__thirdparty.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(thirdparty_ID, status) {
      var SELECTED_ID = thirdparty_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__thirdparty.php?type=changestatus&thirdparty_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#thirdpartyname').parsley();
      var old_thirdparty_name = document.getElementById("encrypt_thirdparty_name").value;
      window.ParsleyValidator.addValidator('checkthirdpartyname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_thirdparty_name.php',
            method: "POST",
            data: {
              thirdparty_name: value,
              old_thirdparty_name: old_thirdparty_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#thirdpartycode').parsley();
      var old_thirdparty_name = document.getElementById("encrypt_thirdpartycode").value;
      window.ParsleyValidator.addValidator('checkthirdpartycode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_thirdparty_code.php',
            method: "POST",
            data: {
              thirdparty_code: value,
              old_thirdparty_code: old_thirdparty_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add thirdparty.', 'error');
    }
