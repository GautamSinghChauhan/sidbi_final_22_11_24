...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "regional_id=$id";

  if (sqlACTIONS("UPDATE", "regional", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'regional.php?code=5'; </script>";
  }
}
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#regionalLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONregional.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "regional_location" //1
            },
            {
              "data": "regional_name" //2
            },
            {
              "data": "regional_contact" //3
            },
            {
              "data": "regional_email" //4
            },
            {
              "data": "regional_state" //5
            },
            {
              "data": "status" //6
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="regionalstatus-' + row.modify + '" id="regionalstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="regionalstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="regionalstatus--' + row.modify + '" id="regionalstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="regionalstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="regional.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__regional.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(regional_id, status) {
      var SELECTED_ID = regional_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__regional.php?type=changestatus&regional_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#regionalname').parsley();
      var old_regional_name = document.getElementById("encrypt_regional_name").value;
      window.ParsleyValidator.addValidator('checkregionalname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_regional_name.php',
            method: "POST",
            data: {
              regional_name: value,
              old_regional_name: old_regional_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#regionalcode').parsley();
      var old_regional_name = document.getElementById("encrypt_regionalcode").value;
      window.ParsleyValidator.addValidator('checkregionalcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_regional_code.php',
            method: "POST",
            data: {
              regional_code: value,
              old_regional_code: old_regional_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add regional.', 'error');
    }
