...
<body>
  ...
  <script>
     if ($("#psigkey_content,#psigkey_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#psigkey_content, textarea#psigkey_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__psigkey.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(psigkey_ID, status) {
      var SELECTED_ID = psigkey_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__psigkey.php?type=changestatus&psigkey_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#psigkeyname').parsley();
      var old_psigkey_name = document.getElementById("encrypt_psigkey_name").value;
      window.ParsleyValidator.addValidator('checkpsigkeyname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigkey_name.php',
            method: "POST",
            data: {
              psigkey_name: value,
              old_psigkey_name: old_psigkey_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#psigkeycode').parsley();
      var old_psigkey_name = document.getElementById("encrypt_psigkeycode").value;
      window.ParsleyValidator.addValidator('checkpsigkeycode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigkey_code.php',
            method: "POST",
            data: {
              psigkey_code: value,
              old_psigkey_code: old_psigkey_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add psigkey.', 'error');
    }
