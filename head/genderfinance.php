...
<body>
  ...
  <script>
     if ($("#genderfinance_content,#genderfinance_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#genderfinance_content, textarea#genderfinance_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#genderfinanceimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__genderfinance.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(genderfinance_ID, status) {
      var SELECTED_ID = genderfinance_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__genderfinance.php?type=changestatus&genderfinance_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#genderfinancename').parsley();
      var old_genderfinance_name = document.getElementById("encrypt_genderfinance_name").value;
      window.ParsleyValidator.addValidator('checkgenderfinancename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_genderfinance_name.php',
            method: "POST",
            data: {
              genderfinance_name: value,
              old_genderfinance_name: old_genderfinance_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#genderfinancecode').parsley();
      var old_genderfinance_name = document.getElementById("encrypt_genderfinancecode").value;
      window.ParsleyValidator.addValidator('checkgenderfinancecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_genderfinance_code.php',
            method: "POST",
            data: {
              genderfinance_code: value,
              old_genderfinance_code: old_genderfinance_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add genderfinance.', 'error');
    }
