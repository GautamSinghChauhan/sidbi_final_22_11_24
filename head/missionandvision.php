  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#missionandvisionLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONmissionandvision.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count"
            },
            {
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="missionandvisionstatus-' + row.modify + '" id="missionandvisionstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="missionandvisionstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="missionandvisionstatus--' + row.modify + '" id="missionandvisionstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="missionandvisionstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="missionandvision.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
      ...
      if ($route == 'add' || $route == 'edit') {  ?>
        ...
        $("#missionandvisionimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__missionandvision.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(missionandvision_id, status) {
      var SELECTED_ID = missionandvision_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__missionandvision.php?type=changestatus&missionandvision_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#missionandvisionname').parsley();
      var old_missionandvision_name = document.getElementById("encrypt_missionandvision_name").value;
      window.ParsleyValidator.addValidator('checkmissionandvisionname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_missionandvision_name.php',
            method: "POST",
            data: {
              missionandvision_name: value,
              old_missionandvision_name: old_missionandvision_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#missionandvisioncode').parsley();
      var old_missionandvision_name = document.getElementById("encrypt_missionandvisioncode").value;
      window.ParsleyValidator.addValidator('checkmissionandvisioncode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_missionandvision_code.php',
            method: "POST",
            data: {
              missionandvision_code: value,
              old_missionandvision_code: old_missionandvision_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add missionandvision.', 'error');
    }
