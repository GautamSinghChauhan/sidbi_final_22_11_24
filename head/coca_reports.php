...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "coca_reports", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'coca_reports.php?code=5'; </script>";
  }
}
...
<body class="page-profile">
	...
	$(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#coca_reportsLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONcoca_reports.php?show=<?php echo $show; ?>&language=en',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "coca_reports_title" //1
            },
            {
              "data": "coca_reports_date" //2
            },
            {
              "data": "status" //3
            },
          ...
          "columnDefs": [{
            ...
            }, 
             {
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="coca_reportstatus-' + row.modify + '" id="coca_reportstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="coca_reportstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="coca_reportstatus--' + row.modify + '" id="coca_reportstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="coca_reportstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            }, {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="coca_reports.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__coca_reports.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(coca_reports_id, status) {
      var SELECTED_ID = coca_reports_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__coca_reports.php?type=changestatus&coca_reports_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
	...
	<?php if ($route == 'add' || $route == 'edit'):  ?>
		...
		$('#coca_reports_last_date').datepicker({
		  dateFormat: 'dd/mm/yy',
		  showOtherMonths: true,
		  selectOtherMonths: true,
		  changeMonth: true,
		  changeYear: true
			...
			$('#submit_upload_document_vehicle_btn').click(function() {
				...
				const newlySelectedDocumentLanguage = $('#coca_reports_document_language_id').val();
		...
		function displayUploadedFiles(selectedDocumentType, selectedFiles) {
			...
			for (let i = 0; i < selectedFiles.length; i++) {
				...
				const selectedDocumentTypeID = $('#coca_reports_document_type_id').text();
				const selectedDocumentLangiageID = $('#coca_reports_document_language_id').val();
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="coca_reports_document_type_id[${counter_image}]" id="coca_reports_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="coca_reports_document_language_id[${counter_image}]" id="coca_reports_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
				...
				$('#coca_reports_document_language_id').find('option:selected').prop('selected', false);
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add coca_reports.', 'error');
    }
