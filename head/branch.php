...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "branch_id=$id";

  if (sqlACTIONS("UPDATE", "branch", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'branch.php?code=5'; </script>";
  }
}
...
<body>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#branchLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONbranch.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "branch_location" //1
            },
            {
              "data": "branch_name" //2
            },
            {
              "data": "branch_contact" //3
            },
            {
              "data": "branch_email" //4
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="branchstatus-' + row.modify + '" id="branchstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="branchstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="branchstatus--' + row.modify + '" id="branchstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="branchstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="branch.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__branch.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(branch_id, status) {
      var SELECTED_ID = branch_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__branch.php?type=changestatus&branch_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#branchname').parsley();
      var old_branch_name = document.getElementById("encrypt_branch_name").value;
      window.ParsleyValidator.addValidator('checkbranchname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_branch_name.php',
            method: "POST",
            data: {
              branch_name: value,
    ...
    $(document).ready(function() {
      $('#branchcode').parsley();
      var old_branch_name = document.getElementById("encrypt_branchcode").value;
      window.ParsleyValidator.addValidator('checkbranchcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_branch_code.php',
            method: "POST",
            data: {
              branch_code: value,
              old_branch_code: old_branch_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add branch.', 'error');
    }
