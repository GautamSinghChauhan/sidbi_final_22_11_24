...
<body>
  ...
  <script>
     if ($("#listofpartners_content,#listofpartners_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#listofpartners_content, textarea#listofpartners_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#listofpartnersimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__listofpartners.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(listofpartners_ID, status) {
      var SELECTED_ID = listofpartners_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__listofpartners.php?type=changestatus&listofpartners_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#listofpartnersname').parsley();
      var old_listofpartners_name = document.getElementById("encrypt_listofpartners_name").value;
      window.ParsleyValidator.addValidator('checklistofpartnersname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_listofpartners_name.php',
            method: "POST",
            data: {
              listofpartners_name: value,
              old_listofpartners_name: old_listofpartners_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#listofpartnerscode').parsley();
      var old_listofpartners_name = document.getElementById("encrypt_listofpartnerscode").value;
      window.ParsleyValidator.addValidator('checklistofpartnerscode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_listofpartners_code.php',
            method: "POST",
            data: {
              listofpartners_code: value,
              old_listofpartners_code: old_listofpartners_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add listofpartners.', 'error');
    }
