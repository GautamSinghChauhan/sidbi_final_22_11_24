...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "workingproduct_id=$id";

  if (sqlACTIONS("UPDATE", "workingproduct", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'workingproduct.php?code=5'; </script>";
  }
}
...
<script>
 if ($("#workingproduct_key,#workingproduct_key_hindi,#workingproduct_eligibility,#workingproduct_eligibility_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#workingproduct_key, textarea#workingproduct_key_hindi, textarea#workingproduct_eligibility, textarea#workingproduct_eligibility_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#workingproductLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONworkingproduct.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "workingproduct_tab" //1
            },
            {
              "data": "workingproduct_tab_hindi" //2
            },
            {
              "data": "status" //3
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="workingproductstatus-' + row.modify + '" id="workingproductstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="workingproductstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="workingproductstatus--' + row.modify + '" id="workingproductstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="workingproductstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="workingproduct.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
      ...
      if ($route == 'add' || $route == 'edit') {  ?>
        ...
        $("#workingproductimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__workingproduct.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(workingproduct_id, status) {
      var SELECTED_ID = workingproduct_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__workingproduct.php?type=changestatus&workingproduct_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#workingproductname').parsley();
      var old_workingproduct_name = document.getElementById("encrypt_workingproduct_name").value;
      window.ParsleyValidator.addValidator('checkworkingproductname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_workingproduct_name.php',
            method: "POST",
            data: {
              workingproduct_name: value,
              old_workingproduct_name: old_workingproduct_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#workingproductcode').parsley();
      var old_workingproduct_name = document.getElementById("encrypt_workingproductcode").value;
      window.ParsleyValidator.addValidator('checkworkingproductcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_workingproduct_code.php',
            method: "POST",
            data: {
              workingproduct_code: value,
              old_workingproduct_code: old_workingproduct_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add workingproduct.', 'error');
    }
