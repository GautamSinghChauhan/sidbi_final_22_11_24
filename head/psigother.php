...
<body>
  ...
  <script>
     if ($("#psigother_content,#psigother_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#psigother_content, textarea#psigother_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#psigotherimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__psigother.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(psigother_ID, status) {
      var SELECTED_ID = psigother_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__psigother.php?type=changestatus&psigother_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#psigothername').parsley();
      var old_psigother_name = document.getElementById("encrypt_psigother_name").value;
      window.ParsleyValidator.addValidator('checkpsigothername', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigother_name.php',
            method: "POST",
            data: {
              psigother_name: value,
              old_psigother_name: old_psigother_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#psigothercode').parsley();
      var old_psigother_name = document.getElementById("encrypt_psigothercode").value;
      window.ParsleyValidator.addValidator('checkpsigothercode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigother_code.php',
            method: "POST",
            data: {
              psigother_code: value,
              old_psigother_code: old_psigother_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add psigother.', 'error');
    }
