...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "other_loans_id=$id";

  if (sqlACTIONS("UPDATE", "other_loans", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'other_loans.php?code=5'; </script>";
  }
}
...
<body class="page-profile">
	...
	$(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#other_loansLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONother_loans.php?show=<?php echo $show; ?>&language=en',
          "columns": [{
              "data": "count" //0
            },
            {
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                return '<div><h6>In English</h6><p>' + row.other_loans_title_eng + '</p></div><div><h6>In Hindi</h6><p>' + row.other_loans_title_hindi +'</p></div>';
              }
            }, 
             {
              "targets": 3,
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="other_loanstatus-' + row.modify + '" id="other_loanstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="other_loanstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="other_loanstatus--' + row.modify + '" id="other_loanstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="other_loanstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            }, {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="other_loans.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__other_loans.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(other_loans_id, status) {
      var SELECTED_ID = other_loans_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__other_loans.php?type=changestatus&other_loans_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
	...
	<?php if ($route == 'add' || $route == 'edit'):  ?>
		...
		$('#other_loans_last_date').datepicker({
		  dateFormat: 'dd/mm/yy',
		  showOtherMonths: true,
		  selectOtherMonths: true,
		  changeMonth: true,
		  changeYear: true
			...
			$('#submit_upload_document_vehicle_btn').click(function() {
				...
				const newlySelectedDocumentLanguage = $('#other_loans_document_language_id').val();
		...
		function displayUploadedFiles(selectedDocumentType, selectedFiles) {
			...
			for (let i = 0; i < selectedFiles.length; i++) {
				...
				const selectedDocumentTypeID = $('#other_loans_document_type_id').text();
				const selectedDocumentLangiageID = $('#other_loans_document_language_id').val();
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="other_loans_document_type_id[${counter_image}]" id="other_loans_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="other_loans_document_language_id[${counter_image}]" id="other_loans_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
				...
				$('#other_loans_document_language_id').find('option:selected').prop('selected', false);
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add other_loans.', 'error');
    }
