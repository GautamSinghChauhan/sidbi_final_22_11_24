...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "projectproduct_id=$id";

  if (sqlACTIONS("UPDATE", "projectproduct", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'projectproduct.php?code=5'; </script>";
  }
}
...
<script>
 if ($("#projectproduct_key,#projectproduct_key_hindi,#projectproduct_eligibility,#projectproduct_eligibility_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#projectproduct_key, textarea#projectproduct_key_hindi, textarea#projectproduct_eligibility, textarea#projectproduct_eligibility_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#projectproductLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONprojectproduct.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "projectproduct_tab" //1
            },
            {
              "data": "projectproduct_tab_hindi" //2
            },
            {
              "data": "status" //3
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="projectproductstatus-' + row.modify + '" id="projectproductstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="projectproductstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="projectproductstatus--' + row.modify + '" id="projectproductstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="projectproductstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="projectproduct.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
      ...
      if ($route == 'add' || $route == 'edit') {  ?>
        ...
        $("#projectproductimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__projectproduct.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(projectproduct_id, status) {
      var SELECTED_ID = projectproduct_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__projectproduct.php?type=changestatus&projectproduct_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#projectproductname').parsley();
      var old_projectproduct_name = document.getElementById("encrypt_projectproduct_name").value;
      window.ParsleyValidator.addValidator('checkprojectproductname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_projectproduct_name.php',
            method: "POST",
            data: {
              projectproduct_name: value,
              old_projectproduct_name: old_projectproduct_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#projectproductcode').parsley();
      var old_projectproduct_name = document.getElementById("encrypt_projectproductcode").value;
      window.ParsleyValidator.addValidator('checkprojectproductcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_projectproduct_code.php',
            method: "POST",
            data: {
              projectproduct_code: value,
              old_projectproduct_code: old_projectproduct_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add projectproduct.', 'error');
    }
