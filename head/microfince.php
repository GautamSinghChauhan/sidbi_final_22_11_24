...
$generateINCLUDE = viewGENERATOR($currentpage, $route);
  ...
  if ($action == "delete" && $id != '') {
    ...
    $sqlWhere = "micro_finance_id =$id";

    if (sqlACTIONS("UPDATE", "microfince", $arrFields, $arrValues, $sqlWhere)) {
      // sqlACTIONS("UPDATE", "microfince", $arrFields, $arrValues, $sqlWhere);
      echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
      echo "<script type='text/javascript'>window.location = 'microfince.php?code=5'; </script>";
    }
  }
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
  if ($("#micro_fince_description,#micro_fince_description_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#micro_fince_description, textarea#micro_fince_description_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
    ...
    $(function() {
      'use strict';
      <?php if ($route == '') : ?>
        $('#microfince_LIST').DataTable({
          // DataTables options here
          'ajax': 'engine/json/JSONmicrofinse.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "micro_finance_descrption" //1
            },
            {
              "data": "micro_finance_hindi_description" //2
            },
            // {
            //   "data": "micro_finance_hindi_description" //3
            // },
            // {
            //   "data": "msme_pluse_hindi_description" //4
            // },
            // {
            //   "data": "microfince_image" //5
            // },
            {
              "data": "status" //6
            },
          ...
          "columnDefs": [{
            ...
            // {
            //   "targets": 1,
            //   "data": "microfince_title",
            //   "searchable": false,
            //   "render": function(data, type, row) {
            //     // Handle cases where data is undefined or null
            //     return '<div><h6>In English</h6><p>' + (data || '') + '</p></div><div><h6>In Hindi</h6><p>' + (row.microfince_title_hindi || '') + '</p></div>';
            //   }
            // },
            {
              "targets": 3,
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="microfince.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(micro_finance_id, status) {
      var SELECTED_ID = micro_finance_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__microfince.php?type=changestatus&micro_finance_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__microfince.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
