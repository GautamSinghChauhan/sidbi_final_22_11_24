...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "career_id=$id";

  if (sqlACTIONS("UPDATE", "careers", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'career.php?code=5'; </script>";
  }
}
...
<body class="page-profile">
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#careerLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONcareer.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
          ...
          "columnDefs": [{
            ...
            "render": function(data, type, row) {
              switch (data) {
                case '1':
                  return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="careerstatus-' + row.modify + '" id="careerstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="careerstatus' + row.modify + '">Yes</label></div>';
                  break;
                case '0':
                  return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="careerstatus--' + row.modify + '" id="careerstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="careerstatus' + row.modify + '">Yes</label></div>';
                  break;
              }
            }
          }, {
            ...
            "render": function(data, type, full, meta) {
              return '<a title="Click to edit" href="career.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              //return '';
            }
          }],
          language: {
      ...
      if ($route == 'archived') {  ?>
        ...
        $('#archivedcareerLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONarchivedcareer.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__career.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(career_id, status) {
      var SELECTED_ID = career_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__career.php?type=changestatus&career_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    <?php if ($route == 'add' || $route == 'edit') :  ?>
      ...
      $('#submit_upload_document_vehicle_btn').click(function() {
        ...
        const newlySelectedDocumentLanguage = $('#career_document_language_id').val();
      ...
      function displayUploadedFiles(selectedDocumentType, selectedFiles) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          const selectedDocumentTypeID = $('#career_document_type_id').val();
          const selectedDocumentLangiageID = $('#career_document_language_id').val();
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="career_document_type_id[${counter_image}]" id="career_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="career_document_language_id[${counter_image}]" id="career_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
										<input type="file" name="document[${counter_image}]" id="uploadDocument_${counter_image}" hidden="" />
									</div>
								</div>
								<div class="text-center mt-2">
									<p class="card-text mb-0 text-center text-break"> ${modifiedFileName} </p>
								</div>
							</div>
						</div>`);
          ...
          $('#career_document_language_id').find('option:selected').prop('selected', false);
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add career.', 'error');
    }
