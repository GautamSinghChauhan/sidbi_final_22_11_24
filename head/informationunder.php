...
$generateINCLUDE = viewGENERATOR($currentpage, $route);
  ...
  if ($action == "delete" && $id != '') {
    ...
    $sqlWhere = "informationunder_id =$id";

    if (sqlACTIONS("UPDATE", "informationunder", $arrFields, $arrValues, $sqlWhere)) {
      // sqlACTIONS("UPDATE", "informationunder", $arrFields, $arrValues, $sqlWhere);
      echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
      echo "<script type='text/javascript'>window.location = 'informationunder.php?code=5'; </script>";
    }
  }
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
  if ($("#informationunder_provision,#informationunder_provision_hindi,#informationunder_detail,#informationunder_detail_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#informationunder_provision, textarea#informationunder_provision_hindi,textarea#informationunder_detail, textarea#informationunder_detail_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
    ...
    $(function() {
      'use strict';
      <?php if ($route == '') : ?>
        $('#informationunderLIST').DataTable({
          // DataTables options here
          'ajax': 'engine/json/JSONinformationunder.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "informationunder_detail" //1
            },
            {
              "data": "informationunder_detail_hindi" //2
            },
            {
              "data": "informationunder_provision" //3
            },
            {
              "data": "informationunder_provision_hindi" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
            ...
            // {
            //   "targets": 1,
            //   "data": "informationunder_detail",
            //   "searchable": false,
            //   "render": function(data, type, row) {
            //     // Handle cases where data is undefined or null
            //     return '<div><h6>In English</h6><p>' + (data || '') + '</p></div><div><h6>In Hindi</h6><p>' + (row.informationunder_detail_hindi || '') + '</p></div>';
            //   }
            // },
            {
              "targets": 5,
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="informationunder.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(informationunder_id, status) {
      var SELECTED_ID = informationunder_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__informationunder.php?type=changestatus&informationunder_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__informationunder.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
