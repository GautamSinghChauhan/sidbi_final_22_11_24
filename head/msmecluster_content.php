...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "msme_cluster", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'msmecluster_content.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') { ?>
        $('#msmeLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONmsmecontent.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "msmecluster_title" //1
            },
            {
              "data": "msmecluster_description" //2
            },
            {
              "data": "msmecluster_hindi_title" //3
            },
            {
              "data": "msmecluster_hindi_description" //4
            },
            {
              "data": "msmecluster_button1" //5
            },
            {
              "data": "status" //6
            },
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="msmecluster_content.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(msme_id, status) {
      var SELECTED_ID = msme_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__msmecluster_content.php?type=changestatus&msme_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__msmecluster_content.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
