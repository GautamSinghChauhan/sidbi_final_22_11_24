...
$list_aboutcontent_datas = sqlQUERY_LABEL("SELECT `msme_pluse_id`, `msme_title`, `msme_hindi_title`, `msme_pluse_description`, `msme_pluse_hindi_description`, `createdby`, `createdon`, `updatedon`, `status`, `deleted` FROM `js_msmepulse`") or die("#1-Unable to get records:" . sqlERROR_LABEL());
...
while ($row = sqlFETCHARRAY_LABEL($list_aboutcontent_datas)) :
    ...
    $data[] = [
        "count" => $counter,
        "msme_title" => $msme_title,
        "msme_hindi_title" => $msme_hindi_title,
        "msme_pluse_description" => $msme_pluse_description,
        "msme_pluse_hindi_description" => $msme_pluse_hindi_description,
        "status" => $status,
        "modify" => $msme_pluse_id
    ];
