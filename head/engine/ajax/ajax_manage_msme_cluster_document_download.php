...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'send_otp') :
        ...
        unset($_SESSION['msme_full_name']);
        unset($_SESSION['msme_occupation']);
        unset($_SESSION['msme_gender']);
        unset($_SESSION['msme_category']);
        unset($_SESSION['msme_email']);
        unset($_SESSION['msme_otp_no']);
        ...
        if (!empty($errors)) :
            ...
            $_SESSION['msme_full_name'] = $_full_name;
            $_SESSION['msme_occupation'] = $_occupation;
            $_SESSION['msme_gender'] = $_gender;
            $_SESSION['msme_category'] = $_category;
            $_SESSION['msme_email'] = $_email;
            $_SESSION['msme_otp_no'] = $get_OTP;
        ...
        if (!empty($errors)) :
            ...
            <body>
                <div class="container">
                    ...
                    <h1 style="justify-items: center;align-items: center;text-align: center;display: grid;">SIDBI MSME OTP
                        Verification</h1>
                    <p>Dear <b>' . $_SESSION['msme_full_name'] . '</b>,</p>
                    <p>Thank you for signing up with [Your Service Name]. We\'re excited to have you join our community!
            ...
            $subject = "SIDBI MSME OTP Verification - $get_OTP";
            $send_from = "$SMTP_EMAIL_SEND_FROM";
            $to = $_SESSION['msme_email']; //$user_email
            $Bcc = 'ariyappan@touchmarkdes.com';
            $cc = '';
            $sender_name = "$SMTP_EMAIL_SEND_NAME";
            SMTP_EMAIL_CONFIG($to, $cc, $send_from, $Bcc, $sender_name, $subject, $message_template);
    ...
    elseif ($_GET['type'] == 'verify_otp') :
        ...
        $msme_OTP_NO = $_SESSION['msme_otp_no'];

        if (empty($_email_otp)) :
            $errors['msme_email_otp_required'] = 'OTP should be Required !!!';
        endif;

        if ($msme_OTP_NO != $_email_otp) :
            $errors['msme_email_otp_invalid'] = 'Entered OTP is Invalid !!!';
        endif;
    ...
    elseif ($_GET['type'] == 'download_document') :
        ...
        $msme_user_captcha = $_POST['msme_captcha'];
        $msme_server_captcha = $_SESSION['msme_server_captcha'];

        if (isset($msme_user_captcha) && ($msme_user_captcha != $msme_server_captcha)) :
            $errors['msme_captcha_verification_failed'] = 'Captcha Verification Failed !!!';
        endif;
        ...
        if (!empty($errors)) :
            ...
            $_full_name = $_SESSION['msme_full_name'];
            $_occupation = $_SESSION['msme_occupation'];
            $_gender = $_SESSION['msme_gender'];
            $_category = $_SESSION['msme_category'];
            $_email = $_SESSION['msme_email'];
            $hidden_DOC_ID = $_POST['hidden_DOC_ID'];

            if ($hidden_DOC_ID == 1) :
                $document_link = SEOURL . 'uploads/announcements/'.$msme_document_1;
            elseif ($hidden_DOC_ID == 2) :
                $document_link = SEOURL . 'uploads/announcements/'.$msme_document_2;
            elseif ($hidden_DOC_ID == 3) :
                $document_link = SEOURL . 'uploads/announcements/'.$msme_document_3;
            elseif ($hidden_DOC_ID == 4) :
                $document_link = SEOURL . 'uploads/announcements/'.$msme_document_4;
            elseif ($hidden_DOC_ID == 5) :
                $document_link = SEOURL . 'uploads/announcements/'.$msme_document_5;
            endif;
            ...
            if (sqlACTIONS('INSERT', "msme_cluster_document_enquiry", $arrFields, $arrValues, '')) :
                $response['result'] = true;
                $response['download_document_url'] = '<a id="download_msme_docs" href="' . $document_link . '" style="display: none;" download>Download File</a>';
                unset($_SESSION['msme_full_name']);
                unset($_SESSION['msme_occupation']);
                unset($_SESSION['msme_gender']);
                unset($_SESSION['msme_category']);
