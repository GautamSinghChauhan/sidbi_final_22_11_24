...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form') :
        ...
        $DOC_ID = $_GET['DOC_ID'];
?>
        <form enctype="multipart/form-data" method="post" accept-charset="utf-8" id="msme_cluster_download_form" data-bv-message="This value is not valid" data-bv-feedbackicons-valid="glyphicon glyphicon-ok" data-bv-feedbackicons-invalid="glyphicon glyphicon-remove" data-bv-feedbackicons-validating="glyphicon glyphicon-refresh">
            <div class="row">
                <div class="col-md-12">
                                    ...
                                    <div id="emailverify-email" class="form-group form-group-emailverify field-email">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <div class="input-group-append">
                                                <button type="button" onclick="sendOTP()" id="email-sendotp-button" class="btn btn-info send_otp_btn" type="button">Send OTP</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="form-group reSetPass">
                        <div class="row">
                            <div class="col-md-6 col-xs-6">
                                ...
                                <input type="text" maxlength="5" autocomplete="off" class="form-control" name="msme_captcha" id="msme_captcha" data-bv-notempty-message="Captcha is required" />
                            </div>
                            <div class="col-md-6">
                                <div class="captcha">
                                    <div class="captchaImg">
                                        <img id="generated_captcha" src="head/engine/ajax/ajax_generateCaptchaImagemsme.php" />
                                    </div>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" disabled id="msme_cluster_download_btn" class="btn btn-primary">Submit</button>
            </div>
        </form>
        ...
        <script>
            ...
            function refreshCaptcha() {
                $.ajax({
                    type: "GET",
                    url: "head/engine/ajax/ajax_generateCaptchaImagemsme.php?refresh=true",
                    dataType: "json",
                    success: function(response) {
                        // Update the src attribute of the captcha image
                        $("#generated_captcha").attr("src", "head/engine/ajax/ajax_generateCaptchaImagemsme.php?" + new Date().getTime());
            ...
            function sendOTP() {
                ...
                $.ajax({
                    type: "POST",
                    url: "head/engine/ajax/ajax_manage_msme_cluster_document_download.php?type=send_otp",
                    data: {
            ...
            function otp_RESENT() {
                $.ajax({
                    type: "POST",
                    url: "head/engine/ajax/ajax_manage_msme_cluster_document_download.php?type=otp_resent",
                    dataType: 'json',
                    success: function(response) {
                        if (!response.success) {
            ...
            function verify_OTP() {
                ...
                $.ajax({
                    type: "POST",
                    url: "head/engine/ajax/ajax_manage_msme_cluster_document_download.php?type=verify_otp",
                    data: {
                    ...
                    success: function(response) {
                        if (!response.success) {
                            if (response.errors.msme_email_otp_required) {
                                showAlert('Error', response.errors.msme_email_otp_required);
                            } else if (response.errors.msme_email_otp_invalid) {
                                showAlert('Error', response.errors.msme_email_otp_invalid);
                            }
                        } else {
                            $('#endtime_div').remove();
                            ...
                            $('#msme_captcha').attr('required', true);
                            $('#show_verified_otp').removeClass('d-none');
                            $('#msme_cluster_download_btn').removeAttr('disabled');
                        }
                    }
                });
            }
            ...
            $('#msme_captcha').on('keyup', function(e) {
                $('#msme_cluster_download_btn').removeAttr('disabled');
            });
            ...
            $(document).ready(function() {
                ...
                //AJAX FORM SUBMIT
                $("#msme_cluster_download_form").submit(function(event) {
                    var form = $('#msme_cluster_download_form')[0];
                    var data = new FormData(form);
                    $(this).find("button[type='submit']").prop('disabled', true);
                    $('.loader-container').show();
                    $.ajax({
                        type: "post",
                        url: 'head/engine/ajax/ajax_manage_msme_cluster_document_download.php?type=download_document',
                        data: data,
                        processData: false,
                        contentType: false,
                        cache: false,
                        ...
                        if (!response.success) {
                            ...
                            if (response.errors.msme_captcha_verification_failed) {
                                showAlert('Error', response.errors.msme_captcha_verification_failed)
                            }
                        } else {
                            //SUCCESS RESPOSNE
                            if (response.result == true) {
                                ...
                                $("#download_msme_docs")[0].click();
                                $('#ajax_show_download_document').remove();
                                form.reset();
                                $('#showMSMECLUSTERMODALFORM').modal('hide');
