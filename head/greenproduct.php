...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "greenproduct_id=$id";

  if (sqlACTIONS("UPDATE", "greenproduct", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'greenproduct.php?code=5'; </script>";
  }
}
...
<script>
 if ($("#greenproduct_key,#greenproduct_key_hindi,#greenproduct_eligibility,#greenproduct_eligibility_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#greenproduct_key, textarea#greenproduct_key_hindi, textarea#greenproduct_eligibility, textarea#greenproduct_eligibility_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#greenproductLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONgreenproduct.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "greenproduct_tab" //1
            },
            {
              "data": "greenproduct_tab_hindi" //2
            },
            {
              "data": "status" //3
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="greenproductstatus-' + row.modify + '" id="greenproductstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="greenproductstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="greenproductstatus--' + row.modify + '" id="greenproductstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="greenproductstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="greenproduct.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
      ...
      if ($route == 'add' || $route == 'edit') {  ?>
        ...
        $("#greenproductimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__greenproduct.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(greenproduct_id, status) {
      var SELECTED_ID = greenproduct_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__greenproduct.php?type=changestatus&greenproduct_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#greenproductname').parsley();
      var old_greenproduct_name = document.getElementById("encrypt_greenproduct_name").value;
      window.ParsleyValidator.addValidator('checkgreenproductname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_greenproduct_name.php',
            method: "POST",
            data: {
              greenproduct_name: value,
              old_greenproduct_name: old_greenproduct_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#greenproductcode').parsley();
      var old_greenproduct_name = document.getElementById("encrypt_greenproductcode").value;
      window.ParsleyValidator.addValidator('checkgreenproductcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_greenproduct_code.php',
            method: "POST",
            data: {
              greenproduct_code: value,
              old_greenproduct_code: old_greenproduct_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add greenproduct.', 'error');
    }
