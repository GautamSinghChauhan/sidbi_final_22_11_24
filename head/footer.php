...
<body>
  ...
  <script>
     if ($("#footer_content,#footer_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#footer_content, textarea#footer_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(document).ready(function() {
      $('#footername').parsley();
      var old_footer_name = document.getElementById("encrypt_footer_name").value;
      window.ParsleyValidator.addValidator('checkfootername', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_footer_name.php',
            method: "POST",
            data: {
              footer_name: value,
              old_footer_name: old_footer_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#footercode').parsley();
      var old_footer_name = document.getElementById("encrypt_footercode").value;
      window.ParsleyValidator.addValidator('checkfootercode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_footer_code.php',
            method: "POST",
            data: {
              footer_code: value,
              old_footer_code: old_footer_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add footer.', 'error');
    }
