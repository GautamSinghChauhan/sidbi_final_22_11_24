...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "videos", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'videos.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') { ?>
        $('#videosLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONvideos.php',
          "columns": [{
              "data": "count" //0
            },
            {
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="videos.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(videos_id, status) {
      var SELECTED_ID = videos_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__videos.php?type=changestatus&videos_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__videos.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
