...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "corporate_governances", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'corporate_governances.php?code=5'; </script>";
  }
}
...
<body class="page-profile">
	...
	$(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#corporate_governancesLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONcorporate_governances.php?show=<?php echo $show; ?>&language=en',
          "columns": [{
              "data": "count" //0
            },
            {
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                return '<div><h6>In English</h6><p>' + row.corporate_governances_title_eng + '</p></div><div><h6>In Hindi</h6><p>' + row.corporate_governances_title_hindi +'</p></div>';
              }
            }, 
             {
              "targets": 3,
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="corporate_governancestatus-' + row.modify + '" id="corporate_governancestatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="corporate_governancestatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="corporate_governancestatus--' + row.modify + '" id="corporate_governancestatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="corporate_governancestatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            }, {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="corporate_governances.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__corporate_governances.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(corporate_governances_id, status) {
      var SELECTED_ID = corporate_governances_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__corporate_governances.php?type=changestatus&corporate_governances_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
	...
	<?php if ($route == 'add' || $route == 'edit'):  ?>
		$('#corporate_governances_date').datepicker({
		  dateFormat: 'dd/mm/yy',
		...
		$('#corporate_governances_last_date').datepicker({
		  dateFormat: 'dd/mm/yy',
		  showOtherMonths: true,
		  selectOtherMonths: true,
		  changeMonth: true,
		  changeYear: true
			...
			$('#submit_upload_document_vehicle_btn').click(function() {
				...
				const newlySelectedDocumentLanguage = $('#corporate_governances_document_language_id').val();
		...
		function displayUploadedFiles(selectedDocumentType, selectedFiles) {
			...
			for (let i = 0; i < selectedFiles.length; i++) {
				...
				const selectedDocumentTypeID = $('#corporate_governances_document_type_id').text();
				const selectedDocumentLangiageID = $('#corporate_governances_document_language_id').val();
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="corporate_governances_document_type_id[${counter_image}]" id="corporate_governances_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="corporate_governances_document_language_id[${counter_image}]" id="corporate_governances_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
				...
				$('#corporate_governances_document_language_id').find('option:selected').prop('selected', false);
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add corporate_governances.', 'error');
    }
