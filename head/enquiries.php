...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "enquiryform_ID=$id";

  if (sqlACTIONS("UPDATE", "js_enquiryform", $arrFields, $arrValues, $sqlWhere)) {
    sqlACTIONS("UPDATE", "js_enquiryform", $arrFields, $arrValues, $sqlWhere);
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'enquiries.php?code=5'; </script>";
  }
}
...
if ($action == "clear_trash") {
  ...
  if (sqlACTIONS("UPDATE", "js_enquiryform", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'enquiries.php?code=6'; </script>";
  }
}
        ...
        <div class="d-sm-flex align-items-center justify-content-between mt-5">
          ...
          <div class="mg-t-20 mg-sm-t-0">
            ...
            if (!in_array($route, array('add', 'edit', 'trash'))) { ?>
              <a href="javascript:;" onclick="show_filterdiv();" id="show_filter" class="btn btn-xs btn-secondary d-none btn-icon mg-r-2"> <i data-feather="filter"></i>Filter</a>
              <!-- <a href="?route=add" class="btn btn-xs btn-success btn-icon"><i data-feather="plus"></i> <?php echo $__create ?></a> -->
            <?php } ?>

            <?php $list_datas = sqlQUERY_LABEL("SELECT `enquiryform_ID` FROM `js_enquiryform` where deleted = '1' order by enquiryform_ID desc") or die("Unable to get trash records:" . sqlERROR_LABEL());

            $fetch_list = sqlNUMOFROW_LABEL($list_datas);

            if ($fetch_list != 0) { ?>
              <a href="enquiries.php?route=trash" class="btn btn-xs btn-warning btn-icon" style="display:show;"><i data-feather="trash"></i> Trash (<?= getTRASHCOUNT('get_trash_count_enquiry_form'); ?>)</a>
            <?php }
            if ($route == 'trash') { ?>
              <a href="enquiries.php" class="btn btn-xs btn-secondary"> Back</a>

              <a href="#" class="btn btn-xs btn-danger" onclick="showTRASHDELETE()"><i data-feather="x"></i> Clear All</a>
            <?php }
            ?>
            <!-- <?php echo "SELECT COUNT(`enquiryform_ID`) FROM `js_enquiryform` WHERE `deleted` = '1';" ?> -->
  ...
  <script>
    ...
    $(function() {
      ...
      <?php if ($route == '' && $formtype == '') { ?>
        ...
        $('#enquiriesLIST').DataTable({
          dom: 'Bfrtip',
          'ajax': 'engine/json/JSONenquiries.php?name=<?= $name; ?>&mobilenumber=<?= $mobilenumber; ?>&email=<?= $email; ?>&since_from=<?= $since_from; ?>&since_to=<?= $since_to; ?>',
          "columns": [{
              "data": "count"
            },
            {
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="enquiries.php?route=preview&formtype=preview&&id=' + data + '" class="btn btn-xs btn-info btn-icon"><i class="fa fa-eye"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-xs btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
          language: {
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__enquiries.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(enquiryform_ID, status) {
      var SELECTED_ID = enquiryform_ID;
      var SELECTED_STATUS = status;
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__enquiries.php?type=changestatus&enquiryform_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    function showTRASHDELETE() {
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__enquiries.php?type=trash_delete', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
