...
<body>
  ...
  <script>
     if ($("#psigcontact_content,#psigcontact_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#psigcontact_content, textarea#psigcontact_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#psigcontactimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__psigcontact.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(psigcontact_ID, status) {
      var SELECTED_ID = psigcontact_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__psigcontact.php?type=changestatus&psigcontact_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#psigcontactname').parsley();
      var old_psigcontact_name = document.getElementById("encrypt_psigcontact_name").value;
      window.ParsleyValidator.addValidator('checkpsigcontactname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigcontact_name.php',
            method: "POST",
            data: {
              psigcontact_name: value,
              old_psigcontact_name: old_psigcontact_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#psigcontactcode').parsley();
      var old_psigcontact_name = document.getElementById("encrypt_psigcontactcode").value;
      window.ParsleyValidator.addValidator('checkpsigcontactcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigcontact_code.php',
            method: "POST",
            data: {
              psigcontact_code: value,
              old_psigcontact_code: old_psigcontact_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add psigcontact.', 'error');
    }
