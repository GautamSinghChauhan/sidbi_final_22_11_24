...
<body>
  ...
  <script>
     if ($("#institutionalgender_content,#institutionalgender_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#institutionalgender_content, textarea#institutionalgender_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#institutionalgenderimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__institutionalgender.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(institutionalgender_ID, status) {
      var SELECTED_ID = institutionalgender_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__institutionalgender.php?type=changestatus&institutionalgender_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#institutionalgendername').parsley();
      var old_institutionalgender_name = document.getElementById("encrypt_institutionalgender_name").value;
      window.ParsleyValidator.addValidator('checkinstitutionalgendername', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_institutionalgender_name.php',
            method: "POST",
            data: {
              institutionalgender_name: value,
              old_institutionalgender_name: old_institutionalgender_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#institutionalgendercode').parsley();
      var old_institutionalgender_name = document.getElementById("encrypt_institutionalgendercode").value;
      window.ParsleyValidator.addValidator('checkinstitutionalgendercode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_institutionalgender_code.php',
            method: "POST",
            data: {
              institutionalgender_code: value,
              old_institutionalgender_code: old_institutionalgender_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add institutionalgender.', 'error');
    }
