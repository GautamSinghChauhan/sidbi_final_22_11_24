...
function getTENDER_DETAILS($language_type, $tender_title, $requesttype)
{
	if ($requesttype == 'get_tender_id') :
		if ($language_type == 'en') :
			$selected_query = sqlQUERY_LABEL("SELECT `tender_id` FROM `tenders` WHERE `tender_url` = '$tender_title'") or die("#STATELABEL-LABEL: GETTING_STATE_LABEL: " . sqlERROR_LABEL());
			if (sqlNUMOFROW_LABEL($selected_query) > 0) :
				while ($fetch_data = sqlFETCHARRAY_LABEL($selected_query)) :
		...
		elseif ($language_type == 'hi') :
			$selected_query = sqlQUERY_LABEL("SELECT `tender_id` FROM `tender_translations` WHERE `url` = '$tender_title'") or die("#STATELABEL-LABEL: GETTING_STATE_LABEL: " . sqlERROR_LABEL());
			if (sqlNUMOFROW_LABEL($selected_query) > 0) :
				while ($fetch_data = sqlFETCHARRAY_LABEL($selected_query)) :
					$tender_id = $fetch_data['tender_id'];
					return $tender_id;
...
function getCAREER_DETAILS($language_type, $career_title, $requesttype)
{
	if ($requesttype == 'get_career_id') :
		if ($language_type == 'en') :
			$selected_query = sqlQUERY_LABEL("SELECT `career_id` FROM `careers` WHERE `career_url` = '$career_title'") or die("#STATELABEL-LABEL: GETTING_STATE_LABEL: " . sqlERROR_LABEL());
			if (sqlNUMOFROW_LABEL($selected_query) > 0) :
				while ($fetch_data = sqlFETCHARRAY_LABEL($selected_query)) :
		...
		elseif ($language_type == 'hi') :
			$selected_query = sqlQUERY_LABEL("SELECT `career_id` FROM `career_translations` WHERE `url` = '$career_title'") or die("#STATELABEL-LABEL: GETTING_STATE_LABEL: " . sqlERROR_LABEL());
			if (sqlNUMOFROW_LABEL($selected_query) > 0) :
				while ($fetch_data = sqlFETCHARRAY_LABEL($selected_query)) :
					$career_id = $fetch_data['career_id'];
					return $career_id;
...
function getTENDERDOCUMENTTYPE($selected_status_id, $requesttype)
{
	/*****************  SELECT OPTION   *****************/
	if ($requesttype == 'select') { ?>
		<option value='1' <?php if ($selected_status_id == '1') {
