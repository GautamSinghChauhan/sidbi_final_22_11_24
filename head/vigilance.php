...
<body>
  ...
  <script>
     if ($("#vigilance_content,#vigilance_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#vigilance_content, textarea#vigilance_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#vigilanceimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__vigilance.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(vigilance_ID, status) {
      var SELECTED_ID = vigilance_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__vigilance.php?type=changestatus&vigilance_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#vigilancename').parsley();
      var old_vigilance_name = document.getElementById("encrypt_vigilance_name").value;
      window.ParsleyValidator.addValidator('checkvigilancename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_vigilance_name.php',
            method: "POST",
            data: {
              vigilance_name: value,
              old_vigilance_name: old_vigilance_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#vigilancecode').parsley();
      var old_vigilance_name = document.getElementById("encrypt_vigilancecode").value;
      window.ParsleyValidator.addValidator('checkvigilancecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_vigilance_code.php',
            method: "POST",
            data: {
              vigilance_code: value,
              old_vigilance_code: old_vigilance_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add vigilance.', 'error');
    }
