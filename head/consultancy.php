...
<body>
  ...
  <script>
     if ($("#consultancy_content,#consultancy_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#consultancy_content, textarea#consultancy_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#consultancyimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__consultancy.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(consultancy_ID, status) {
      var SELECTED_ID = consultancy_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__consultancy.php?type=changestatus&consultancy_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#consultancyname').parsley();
      var old_consultancy_name = document.getElementById("encrypt_consultancy_name").value;
      window.ParsleyValidator.addValidator('checkconsultancyname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_consultancy_name.php',
            method: "POST",
            data: {
              consultancy_name: value,
              old_consultancy_name: old_consultancy_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#consultancycode').parsley();
      var old_consultancy_name = document.getElementById("encrypt_consultancycode").value;
      window.ParsleyValidator.addValidator('checkconsultancycode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_consultancy_code.php',
            method: "POST",
            data: {
              consultancy_code: value,
              old_consultancy_code: old_consultancy_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add consultancy.', 'error');
    }
