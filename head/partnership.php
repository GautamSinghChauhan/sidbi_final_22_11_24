...
<body>
  ...
  <script>
     if ($("#partnership_content,#partnership_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#partnership_content, textarea#partnership_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#partnershipimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__partnership.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(partnership_ID, status) {
      var SELECTED_ID = partnership_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__partnership.php?type=changestatus&partnership_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#partnershipname').parsley();
      var old_partnership_name = document.getElementById("encrypt_partnership_name").value;
      window.ParsleyValidator.addValidator('checkpartnershipname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_partnership_name.php',
            method: "POST",
            data: {
              partnership_name: value,
              old_partnership_name: old_partnership_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#partnershipcode').parsley();
      var old_partnership_name = document.getElementById("encrypt_partnershipcode").value;
      window.ParsleyValidator.addValidator('checkpartnershipcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_partnership_code.php',
            method: "POST",
            data: {
              partnership_code: value,
              old_partnership_code: old_partnership_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add partnership.', 'error');
    }
