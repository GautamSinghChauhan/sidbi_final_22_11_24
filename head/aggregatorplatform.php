...
<body>
  ...
  <script>
     if ($("#aggregatorplatform_content,#aggregatorplatform_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#aggregatorplatform_content, textarea#aggregatorplatform_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#aggregatorplatformimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__aggregatorplatform.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(aggregatorplatform_ID, status) {
      var SELECTED_ID = aggregatorplatform_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__aggregatorplatform.php?type=changestatus&aggregatorplatform_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#aggregatorplatformname').parsley();
      var old_aggregatorplatform_name = document.getElementById("encrypt_aggregatorplatform_name").value;
      window.ParsleyValidator.addValidator('checkaggregatorplatformname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_aggregatorplatform_name.php',
            method: "POST",
            data: {
              aggregatorplatform_name: value,
              old_aggregatorplatform_name: old_aggregatorplatform_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#aggregatorplatformcode').parsley();
      var old_aggregatorplatform_name = document.getElementById("encrypt_aggregatorplatformcode").value;
      window.ParsleyValidator.addValidator('checkaggregatorplatformcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_aggregatorplatform_code.php',
            method: "POST",
            data: {
              aggregatorplatform_code: value,
              old_aggregatorplatform_code: old_aggregatorplatform_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add aggregatorplatform.', 'error');
    }
