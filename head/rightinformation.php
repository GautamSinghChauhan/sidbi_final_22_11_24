...
<body>
  ...
  <script>
     if ($("#rightinformation_content,#rightinformation_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#rightinformation_content, textarea#rightinformation_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#rightinformationimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__rightinformation.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(rightinformation_ID, status) {
      var SELECTED_ID = rightinformation_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__rightinformation.php?type=changestatus&rightinformation_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#rightinformationname').parsley();
      var old_rightinformation_name = document.getElementById("encrypt_rightinformation_name").value;
      window.ParsleyValidator.addValidator('checkrightinformationname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_rightinformation_name.php',
            method: "POST",
            data: {
              rightinformation_name: value,
              old_rightinformation_name: old_rightinformation_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#rightinformationcode').parsley();
      var old_rightinformation_name = document.getElementById("encrypt_rightinformationcode").value;
      window.ParsleyValidator.addValidator('checkrightinformationcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_rightinformation_code.php',
            method: "POST",
            data: {
              rightinformation_code: value,
              old_rightinformation_code: old_rightinformation_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add rightinformation.', 'error');
    }
