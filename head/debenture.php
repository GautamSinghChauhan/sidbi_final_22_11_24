...
<body>
  ...
  <script>
    if ($("#debenture_content,#debenture_content_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#debenture_content, textarea#debenture_content_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
    ...
    $(function() {
      ...
      $("#debentureimage").change(function() {
        uploadImage(this);
      });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__debenture.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(debenture_ID, status) {
      var SELECTED_ID = debenture_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__debenture.php?type=changestatus&debenture_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#debenturename').parsley();
      var old_debenture_name = document.getElementById("encrypt_debenture_name").value;
      window.ParsleyValidator.addValidator('checkdebenturename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_debenture_name.php',
            method: "POST",
            data: {
              debenture_name: value,
              old_debenture_name: old_debenture_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#debenturecode').parsley();
      var old_debenture_name = document.getElementById("encrypt_debenturecode").value;
      window.ParsleyValidator.addValidator('checkdebenturecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_debenture_code.php',
            method: "POST",
            data: {
              debenture_code: value,
              old_debenture_code: old_debenture_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add debenture.', 'error');
    }
