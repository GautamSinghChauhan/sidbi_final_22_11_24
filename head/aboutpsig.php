...
<body>
  ...
  <script>
     if ($("#aboutpsig_content,#aboutpsig_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#aboutpsig_content, textarea#aboutpsig_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#aboutpsigimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__aboutpsig.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(aboutpsig_ID, status) {
      var SELECTED_ID = aboutpsig_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__aboutpsig.php?type=changestatus&aboutpsig_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#aboutpsigname').parsley();
      var old_aboutpsig_name = document.getElementById("encrypt_aboutpsig_name").value;
      window.ParsleyValidator.addValidator('checkaboutpsigname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_aboutpsig_name.php',
            method: "POST",
            data: {
              aboutpsig_name: value,
              old_aboutpsig_name: old_aboutpsig_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#aboutpsigcode').parsley();
      var old_aboutpsig_name = document.getElementById("encrypt_aboutpsigcode").value;
      window.ParsleyValidator.addValidator('checkaboutpsigcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_aboutpsig_code.php',
            method: "POST",
            data: {
              aboutpsig_code: value,
              old_aboutpsig_code: old_aboutpsig_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add aboutpsig.', 'error');
    }
