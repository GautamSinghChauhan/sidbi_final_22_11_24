...
<body>
  ...
  <script>
     if ($("#upscaling_content,#upscaling_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#upscaling_content, textarea#upscaling_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#upscalingimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__upscaling.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(upscaling_ID, status) {
      var SELECTED_ID = upscaling_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__upscaling.php?type=changestatus&upscaling_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#upscalingname').parsley();
      var old_upscaling_name = document.getElementById("encrypt_upscaling_name").value;
      window.ParsleyValidator.addValidator('checkupscalingname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_upscaling_name.php',
            method: "POST",
            data: {
              upscaling_name: value,
              old_upscaling_name: old_upscaling_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#upscalingcode').parsley();
      var old_upscaling_name = document.getElementById("encrypt_upscalingcode").value;
      window.ParsleyValidator.addValidator('checkupscalingcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_upscaling_code.php',
            method: "POST",
            data: {
              upscaling_code: value,
              old_upscaling_code: old_upscaling_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add upscaling.', 'error');
    }
