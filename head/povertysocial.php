...
<body>
  ...
  <script>
     if ($("#povertysocial_content,#povertysocial_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#povertysocial_content, textarea#povertysocial_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#povertysocialimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__povertysocial.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(povertysocial_ID, status) {
      var SELECTED_ID = povertysocial_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__povertysocial.php?type=changestatus&povertysocial_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#povertysocialname').parsley();
      var old_povertysocial_name = document.getElementById("encrypt_povertysocial_name").value;
      window.ParsleyValidator.addValidator('checkpovertysocialname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_povertysocial_name.php',
            method: "POST",
            data: {
              povertysocial_name: value,
              old_povertysocial_name: old_povertysocial_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#povertysocialcode').parsley();
      var old_povertysocial_name = document.getElementById("encrypt_povertysocialcode").value;
      window.ParsleyValidator.addValidator('checkpovertysocialcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_povertysocial_code.php',
            method: "POST",
            data: {
              povertysocial_code: value,
              old_povertysocial_code: old_povertysocial_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add povertysocial.', 'error');
    }
