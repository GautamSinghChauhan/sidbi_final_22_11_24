...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "machinerytest", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'machinerytest.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') { ?>
        $('#machinerytestLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONmachinerytest.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "machinerytest_content" //1
            },
            {
              "data": "machinerytest_content_hindi" //2
            },
            {
              "data": "machinerytest_image" //3
            },
            {
              "data": "machinerytest_link" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="machinerytest.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(machinerytest_id, status) {
      var SELECTED_ID = machinerytest_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__machinerytest.php?type=changestatus&machinerytest_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__machinerytest.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
