...
<body>
  ...
  <script>
     if ($("#psigfinance_content,#psigfinance_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#psigfinance_content, textarea#psigfinance_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#psigfinanceimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__psigfinance.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(psigfinance_ID, status) {
      var SELECTED_ID = psigfinance_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__psigfinance.php?type=changestatus&psigfinance_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#psigfinancename').parsley();
      var old_psigfinance_name = document.getElementById("encrypt_psigfinance_name").value;
      window.ParsleyValidator.addValidator('checkpsigfinancename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigfinance_name.php',
            method: "POST",
            data: {
              psigfinance_name: value,
              old_psigfinance_name: old_psigfinance_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#psigfinancecode').parsley();
      var old_psigfinance_name = document.getElementById("encrypt_psigfinancecode").value;
      window.ParsleyValidator.addValidator('checkpsigfinancecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigfinance_code.php',
            method: "POST",
            data: {
              psigfinance_code: value,
              old_psigfinance_code: old_psigfinance_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add psigfinance.', 'error');
    }
