...
<body>
  ...
  <script>
     if ($("#appellate_content,#appellate_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#appellate_content, textarea#appellate_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#appellateimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__appellate.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(appellate_ID, status) {
      var SELECTED_ID = appellate_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__appellate.php?type=changestatus&appellate_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#appellatename').parsley();
      var old_appellate_name = document.getElementById("encrypt_appellate_name").value;
      window.ParsleyValidator.addValidator('checkappellatename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_appellate_name.php',
            method: "POST",
            data: {
              appellate_name: value,
              old_appellate_name: old_appellate_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#appellatecode').parsley();
      var old_appellate_name = document.getElementById("encrypt_appellatecode").value;
      window.ParsleyValidator.addValidator('checkappellatecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_appellate_code.php',
            method: "POST",
            data: {
              appellate_code: value,
              old_appellate_code: old_appellate_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add appellate.', 'error');
    }
