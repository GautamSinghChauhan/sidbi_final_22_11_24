...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "unlock_growthproduct", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'growthproduct.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') { ?>
        $('#growthproductLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONgrowthproduct.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "unlockgrowthproduct_title" //1
            },
            {
              "data": "unlockgrowthproduct_hindi_title" //2
            },
            {
              "data": "unlockgrowthproduct_image" //3
            },
            {
              "data": "status" //4
            },
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="growthproduct.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(growthproduct_id, status) {
      var SELECTED_ID = growthproduct_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__growthproduct.php?type=changestatus&growthproduct_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__growthproduct.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
