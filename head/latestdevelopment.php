...
$generateINCLUDE = viewGENERATOR($currentpage, $route);
  ...
  if ($action == "delete" && $id != '') {
    ...
    $sqlWhere = "latestdevelopment_id =$id";

    if (sqlACTIONS("UPDATE", "latestdevelopment", $arrFields, $arrValues, $sqlWhere)) {
      // sqlACTIONS("UPDATE", "latestdevelopment", $arrFields, $arrValues, $sqlWhere);
      echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
      echo "<script type='text/javascript'>window.location = 'latestdevelopment.php?code=5'; </script>";
    }
  }
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
  // if ($("#latestdevelopment_description,#latestdevelopment_description_hindi,#latestdevelopmentimpact_description,#latestdevelopmentimpact_description_hindi").length > 0) {
  //     tinymce.init({
  //       selector: "textarea#latestdevelopment_description, textarea#latestdevelopment_description_hindi, textarea#latestdevelopmentimpact_description, textarea#latestdevelopmentimpact_description_hindi",
  //       theme: "modern",
  //       menubar: false,
  //       min_height: 300,
  ...
  //   }
    ...
    $(function() {
      'use strict';
      <?php if ($route == '') : ?>
        $('#latestdevelopmentLIST').DataTable({
          // DataTables options here
          'ajax': 'engine/json/JSONlatestdevelopment.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "latestdevelopment_title" //1
            },
            {
              "data": "latestdevelopment_title_hindi" //2
            },
            {
              "data": "latestdevelopment_image" //3
            },
            {
              "data": "status" //4
            },
          ...
          "columnDefs": [{
            ...
            // {
            //   "targets": 1,
            //   "data": "latestdevelopment_title",
            //   "searchable": false,
            //   "render": function(data, type, row) {
            //     // Handle cases where data is undefined or null
            //     return '<div><h6>In English</h6><p>' + (data || '') + '</p></div><div><h6>In Hindi</h6><p>' + (row.latestdevelopment_title_hindi || '') + '</p></div>';
            //   }
            // },
            {
              "targets": 4,
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="latestdevelopment.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
...
<?php endif; ?>
    ...
    function togglestatusITEM(latestdevelopment_id, status) {
      var SELECTED_ID = latestdevelopment_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__latestdevelopment.php?type=changestatus&latestdevelopment_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__latestdevelopment.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
