...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "boardofdirector_ID=$id";

  if (sqlACTIONS("UPDATE", "js_boardofdirector", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'boardofdirector.php?code=5'; </script>";
  }
}
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#boardofdirectorLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONboardofdirector.php?show=<?php echo $show; ?>',
          "columns": [{
              "data": "count"
            },
            {
            ...
            {
              "data": "director_shortdescription"
            },
            {
              "data": "status"
            },
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="boardofdirectorstatus-' + row.modify + '" id="boardofdirectorstatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="boardofdirectorstatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="boardofdirectorstatus--' + row.modify + '" id="boardofdirectorstatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="boardofdirectorstatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="boardofdirector.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
      ...
      if ($route == 'add' || $route == 'edit') {  ?>
        ...
        $("#boardofdirectorimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__boardofdirector.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(boardofdirector_id, status) {
      var SELECTED_ID = boardofdirector_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__boardofdirector.php?type=changestatus&boardofdirector_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#boardofdirectorname').parsley();
      var old_boardofdirector_name = document.getElementById("encrypt_boardofdirector_name").value;
      window.ParsleyValidator.addValidator('checkboardofdirectorname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_boardofdirector_name.php',
            method: "POST",
            data: {
              boardofdirector_name: value,
              old_boardofdirector_name: old_boardofdirector_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#boardofdirectorcode').parsley();
      var old_boardofdirector_name = document.getElementById("encrypt_boardofdirectorcode").value;
      window.ParsleyValidator.addValidator('checkboardofdirectorcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_boardofdirector_code.php',
            method: "POST",
            data: {
              boardofdirector_code: value,
              old_boardofdirector_code: old_boardofdirector_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add boardofdirector.', 'error');
    }
