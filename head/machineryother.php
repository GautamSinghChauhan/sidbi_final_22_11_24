...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "machineryother", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'machineryother.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') { 
        ...
        $('#machineryotherLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONmachineryother.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "machineryother_name" //1
            },
            {
              "data": "machineryother_name_hindi" //2
            },
            {
              "data": "machineryother_image" //3
            },
            {
              "data": "machineryother_link" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="machineryother.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(machineryother_id, status) {
      var SELECTED_ID = machineryother_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__machineryother.php?type=changestatus&machineryother_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__machineryother.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
