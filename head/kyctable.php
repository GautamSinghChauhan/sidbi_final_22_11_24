...
$generateINCLUDE = viewGENERATOR($currentpage, $route);
  ...
  if ($action == "delete" && $id != '') {
    ...
    $sqlWhere = "kyctable_id =$id";

    if (sqlACTIONS("UPDATE", "kyctable", $arrFields, $arrValues, $sqlWhere)) {
      // sqlACTIONS("UPDATE", "kyctable", $arrFields, $arrValues, $sqlWhere);
      echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
      echo "<script type='text/javascript'>window.location = 'kyctable.php?code=5'; </script>";
    }
  }
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
  if ($("#kyctable_notice,#kyctable_notice_hindi,#kyctable_feature,#kyctable_feature_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#kyctable_notice, textarea#kyctable_notice_hindi,textarea#kyctable_feature, textarea#kyctable_feature_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
    ...
    $(function() {
      'use strict';
      <?php if ($route == '') : ?>
        $('#kyctableLIST').DataTable({
          // DataTables options here
          'ajax': 'engine/json/JSONkyctable.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "kyctable_feature" //1
            },
            {
              "data": "kyctable_feature_hindi" //2
            },
            {
              "data": "kyctable_notice" //3
            },
            {
              "data": "kyctable_notice_hindi" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
            ...
            // {
            //   "targets": 1,
            //   "data": "kyctable_feature",
            //   "searchable": false,
            //   "render": function(data, type, row) {
            //     // Handle cases where data is undefined or null
            //     return '<div><h6>In English</h6><p>' + (data || '') + '</p></div><div><h6>In Hindi</h6><p>' + (row.kyctable_feature_hindi || '') + '</p></div>';
            //   }
            // },
            {
              "targets": 5,
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="kyctable.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(kyctable_id, status) {
      var SELECTED_ID = kyctable_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__kyctable.php?type=changestatus&kyctable_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__kyctable.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
