...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "greenother", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'greenother.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') { 
        ...
        $('#greenotherLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONgreenother.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "greenother_name" //1
            },
            {
              "data": "greenother_name_hindi" //2
            },
            {
              "data": "greenother_image" //3
            },
            {
              "data": "greenother_link" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="greenother.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(greenother_id, status) {
      var SELECTED_ID = greenother_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__greenother.php?type=changestatus&greenother_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__greenother.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
