...
<body>
  ...
  <script>
     if ($("#contactcenter_content,#contactcenter_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#contactcenter_content, textarea#contactcenter_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#contactcenterimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__contactcenter.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(contactcenter_ID, status) {
      var SELECTED_ID = contactcenter_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__contactcenter.php?type=changestatus&contactcenter_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#contactcentername').parsley();
      var old_contactcenter_name = document.getElementById("encrypt_contactcenter_name").value;
      window.ParsleyValidator.addValidator('checkcontactcentername', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_contactcenter_name.php',
            method: "POST",
            data: {
              contactcenter_name: value,
              old_contactcenter_name: old_contactcenter_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#contactcentercode').parsley();
      var old_contactcenter_name = document.getElementById("encrypt_contactcentercode").value;
      window.ParsleyValidator.addValidator('checkcontactcentercode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_contactcenter_code.php',
            method: "POST",
            data: {
              contactcenter_code: value,
              old_contactcenter_code: old_contactcenter_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add contactcenter.', 'error');
    }
