...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "financial_reports", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'financialreport.php?code=5'; </script>";
  }
}
...
<body class="page-profile">
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#financialreportLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONfinancialreport.php?show=<?php echo $show; ?>&language=en',
          "columns": [{
              "data": "count" //0
            },
            {
          ...
          "columnDefs": [{
              ...
              "targets": 1,
              "data": "financial_report_title",
              "searchable": false,
              "render": function(data, type, row) {
                return '<div><h6>In English</h6><p>' + row.financial_report_title_eng + '</p></div><div><h6>In Hindi</h6><p>' + row.financial_report_title_hindi + '</p></div>';
              }
            },
            // {
            //   "targets": 3,
            //   "data": "financial_report_eng_doc",
            //   "searchable": false,
            //   "render": function(data, type, row) {
            //     return '<div><h6>In English</h6><p>' + data + '</p></div><div><h6>In Hindi</h6><p>' + row.financial_report_title_hindi + '</p></div>';
            //   }
            // },
            // {
            //   "targets": 4,
            //   "data": "financial_report_hin_doc",
            //   "searchable": false,
            //   "render": function(data, type, row) {
            //     return '<div><h6>In English</h6><p>' + data + '</p></div><div><h6>In Hindi</h6><p>' + row.financial_report_title_hindi + '</p></div>';
            //   }
            // },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="financialreport.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__financialreport.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(financial_report_id, status) {
      var SELECTED_ID = financial_report_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__financialreport.php?type=changestatus&financial_report_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    <?php if ($route == 'add' || $route == 'edit') :  ?>
      ...
      // $('#financialreport_last_date').datepicker({
      //   dateFormat: 'dd/mm/yy',
      //   showOtherMonths: true,
      //   selectOtherMonths: true,
      //   changeMonth: true,
      ...
      $('#submit_upload_document_vehicle_btn').click(function() {
        ...
        const newlySelectedDocumentLanguage = $('#financialreport_document_language_id').val();
      ...
      function displayUploadedFiles(selectedDocumentType, selectedFiles) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          const selectedDocumentTypeID = $('#financialreport_document_type_id').text();
          const selectedDocumentLangiageID = $('#financialreport_document_language_id').val();
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="financialreport_document_type_id[${counter_image}]" id="financialreport_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="financialreport_document_language_id[${counter_image}]" id="financialreport_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
          ...
          $('#financialreport_document_language_id').find('option:selected').prop('selected', false);
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add financial report.', 'error');
    }
