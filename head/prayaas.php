...
<body>
  ...
  <script>
     if ($("#prayaas_content,#prayaas_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#prayaas_content, textarea#prayaas_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__prayaas.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(prayaas_ID, status) {
      var SELECTED_ID = prayaas_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__prayaas.php?type=changestatus&prayaas_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#prayaasname').parsley();
      var old_prayaas_name = document.getElementById("encrypt_prayaas_name").value;
      window.ParsleyValidator.addValidator('checkprayaasname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_prayaas_name.php',
            method: "POST",
            data: {
              prayaas_name: value,
              old_prayaas_name: old_prayaas_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#prayaascode').parsley();
      var old_prayaas_name = document.getElementById("encrypt_prayaascode").value;
      window.ParsleyValidator.addValidator('checkprayaascode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_prayaas_code.php',
            method: "POST",
            data: {
              prayaas_code: value,
              old_prayaas_code: old_prayaas_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add prayaas.', 'error');
    }
