...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "investor_grievances", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'investor_grievances.php?code=5'; </script>";
  }
}
...
<body class="page-profile">
	...
	$(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#investor_grievancesLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONinvestor_grievances.php?show=<?php echo $show; ?>&language=en',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "investor_grievances_title" //1
            },
            {
              "data": "investor_grievances_date" //2
            },
            {
              "data": "status" //3
            },
          ...
          "columnDefs": [{
              ...
              "targets": 1,
              "data": "investor_grievances_title",
              "searchable": false,
            ...
            }, 
             {
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="investor_grievancestatus-' + row.modify + '" id="investor_grievancestatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="investor_grievancestatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="investor_grievancestatus--' + row.modify + '" id="investor_grievancestatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="investor_grievancestatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            }, {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="investor_grievances.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__investor_grievances.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(investor_grievances_id, status) {
      var SELECTED_ID = investor_grievances_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__investor_grievances.php?type=changestatus&investor_grievances_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
	...
	<?php if ($route == 'add' || $route == 'edit'):  ?>
		$('#investor_grievances_date').datepicker({
		  dateFormat: 'dd/mm/yy',
		...
		$('#investor_grievances_last_date').datepicker({
		  dateFormat: 'dd/mm/yy',
		  showOtherMonths: true,
		  selectOtherMonths: true,
		  changeMonth: true,
		  changeYear: true
			...
			$('#submit_upload_document_vehicle_btn').click(function() {
				...
				const newlySelectedDocumentLanguage = $('#investor_grievances_document_language_id').val();
		...
		function displayUploadedFiles(selectedDocumentType, selectedFiles) {
			...
			for (let i = 0; i < selectedFiles.length; i++) {
				...
				const selectedDocumentTypeID = $('#investor_grievances_document_type_id').text();
				const selectedDocumentLangiageID = $('#investor_grievances_document_language_id').val();
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="investor_grievances_document_type_id[${counter_image}]" id="investor_grievances_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="investor_grievances_document_language_id[${counter_image}]" id="investor_grievances_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
				...
				$('#investor_grievances_document_language_id').find('option:selected').prop('selected', false);
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add investor_grievances.', 'error');
    }
