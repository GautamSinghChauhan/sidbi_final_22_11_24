...
<body>
  ...
  <script>
     if ($("#government_content,#government_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#government_content, textarea#government_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#governmentimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__government.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(government_ID, status) {
      var SELECTED_ID = government_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__government.php?type=changestatus&government_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#governmentname').parsley();
      var old_government_name = document.getElementById("encrypt_government_name").value;
      window.ParsleyValidator.addValidator('checkgovernmentname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_government_name.php',
            method: "POST",
            data: {
              government_name: value,
              old_government_name: old_government_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#governmentcode').parsley();
      var old_government_name = document.getElementById("encrypt_governmentcode").value;
      window.ParsleyValidator.addValidator('checkgovernmentcode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_government_code.php',
            method: "POST",
            data: {
              government_code: value,
              old_government_code: old_government_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add government.', 'error');
    }
