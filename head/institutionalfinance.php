...
<body>
  ...
  <script>
     if ($("#institutionalfinance_content,#institutionalfinance_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#institutionalfinance_content, textarea#institutionalfinance_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#institutionalfinanceimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__institutionalfinance.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(institutionalfinance_ID, status) {
      var SELECTED_ID = institutionalfinance_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__institutionalfinance.php?type=changestatus&institutionalfinance_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#institutionalfinancename').parsley();
      var old_institutionalfinance_name = document.getElementById("encrypt_institutionalfinance_name").value;
      window.ParsleyValidator.addValidator('checkinstitutionalfinancename', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_institutionalfinance_name.php',
            method: "POST",
            data: {
              institutionalfinance_name: value,
              old_institutionalfinance_name: old_institutionalfinance_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#institutionalfinancecode').parsley();
      var old_institutionalfinance_name = document.getElementById("encrypt_institutionalfinancecode").value;
      window.ParsleyValidator.addValidator('checkinstitutionalfinancecode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_institutionalfinance_code.php',
            method: "POST",
            data: {
              institutionalfinance_code: value,
              old_institutionalfinance_code: old_institutionalfinance_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add institutionalfinance.', 'error');
    }
