...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "js_innerpages", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'innerpage.php?code=5'; </script>";
  }
}
...
<body class="page-profile">
    ...
    <div class="content pd-0">
        ...
        <div class="modal fade" id="deleteDATA" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel2"
        ...
        <!-- onclick spinner -->
        <div class="modal fade effect-scale show" id="pleasewait-loader" data-backdrop="static" data-keyboard="false"
    ...
    <script>
    if ($("#innerpage_content,#innerpage_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#innerpage_content, textarea#innerpage_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    //     pdfDiv.innerHTML = '<div class="row"><div class="col-lg-5"><div id="image-upload" class="file-upload text-left"><label for="image">Choose Image:</label><input type="file" id="image" name="image" class="form-control"></div></div><div class="col-lg-5"><div id="image-upload" class="file-upload text-left"><label for="image">Choose Image:</label><input type="file" id="image" name="image" class="form-control"></div></div><div class="col-lg-2"><button id="addBtn" type="button" onclick="removePDF(this) class= btn btn-danger">Remove</button></div></div>';
    //     document.getElementById('pdfButton').appendChild(pdfDiv);
    // }
    ...
    //     </div>
    //     <div class="col-lg-2">
    //         <button type="button" onclick="removePDF(this)" class="btn btn-danger">Remove</button>
    //     </div>
    // `;
    //     document.getElementById('pdfButton').appendChild(pdfDiv);
    // }
...
`;
    ...
    const innerpageTitleInput = document.getElementById("innerpage_title");
    ...
    $(function() {
        ...
        $('#innerpageLIST').DataTable({
            //responsive: true,
            'ajax': 'engine/json/JSONinnerpage.php',
            "columns": [{
                    "data": "count" //0
                },
                {
                    "data": "innerpage_title" //1
                },
                {
                    "data": "status" //2
                },
            ...
            "columnDefs": [{
                ...
                "render": function(data, type, full, meta) {
                    return '<a title="Click to edit" href="innerpage.php?route=edit&id=' +
                        data +
                        '" class="btn btn-info btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' +
                        data +
        ...
        //   $('#innerpage_attachementLIST').DataTable({
        // 	//responsive: true,
        // 	'ajax': 'engine/json/JSONinnerpage_attachementLIST.php?innerpage_id=<?= $id; ?>',
        // 	"columns": [{
        // 			data: "count"
        // 		}, //0
        // 		{
        ...
        // 				return '<a target="blank" href="<?php SITEHOME ?>uploads/' + data + '" class="img-fluid img wd-100 ht-100" src="<?php SITEHOME ?>/uploads/' + data + '">data</a>'
        // 			}
        // 		},
        // 		{
        // 			"targets": 2,
        ...
        // 			"searchable": false,
        // 			"render": function(data, type, full, meta) {
        // 				return '<a href="javascript:void(0);" onClick ="confirmREMOVEPODOCUMENTS(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
        // 				//return '';
        // 			}
        // 		}
        // 	],
    ...
    function deleteITEM(deleting_id) {
        ...
        $('.receiving-delete-data').load('view/x__innerpage.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(innerpage_id, status) {
        var SELECTED_ID = innerpage_id;
        var SELECTED_STATUS = status;
        //alert(SELECTED_ID);
        $('#pleasewait-loader').show();
        $('.receiving-delete-data').load('view/x__innerpage.php?type=changestatus&innerpage_id=' + SELECTED_ID +
    ...
    $('#submit_upload_document_vehicle_btn').click(function() {
        ...
        const newlySelectedDocumentLanguage = $('#tender_document_language_id').val();
    ...
    function displayUploadedFiles(selectedDocumentType, selectedFiles) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            const selectedDocumentTypeID = $('#tender_document_type_id').text();
            const selectedDocumentLangiageID = $('#tender_document_language_id').val();
            ...
            reader.onload = (function(file) {
                return function(e) {
                    ...
                    previewHtml = '<iframe src="' + image_src +
                        '" width="600" height="400" frameborder="0" class="d-block w-100 h-100 rounded"></iframe>';
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="tender_document_type_id[${counter_image}]" id="tender_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="tender_document_language_id[${counter_image}]" id="tender_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
            ...
            $('#tender_document_language_id').find('option:selected').prop('selected', false);
