...
<body>
  ...
  <script>
    if ($("#cheifgrievances_content,#cheifgrievances_content_hindi").length > 0) {
      tinymce.init({
        selector: "textarea#cheifgrievances_content, textarea#cheifgrievances_content_hindi",
        theme: "modern",
        menubar: false,
        min_height: 300,
    ...
    $(function() {
      ...
      $("#cheifgrievancesimage").change(function() {
        uploadImage(this);
      });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__cheifgrievances.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(cheifgrievances_ID, status) {
      var SELECTED_ID = cheifgrievances_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__cheifgrievances.php?type=changestatus&cheifgrievances_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#cheifgrievancesname').parsley();
      var old_cheifgrievances_name = document.getElementById("encrypt_cheifgrievances_name").value;
      window.ParsleyValidator.addValidator('checkcheifgrievancesname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_cheifgrievances_name.php',
            method: "POST",
            data: {
              cheifgrievances_name: value,
              old_cheifgrievances_name: old_cheifgrievances_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#cheifgrievancescode').parsley();
      var old_cheifgrievances_name = document.getElementById("encrypt_cheifgrievancescode").value;
      window.ParsleyValidator.addValidator('checkcheifgrievancescode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_cheifgrievances_code.php',
            method: "POST",
            data: {
              cheifgrievances_code: value,
              old_cheifgrievances_code: old_cheifgrievances_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add cheifgrievances.', 'error');
    }
