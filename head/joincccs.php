...
$generateINCLUDE = viewGENERATOR($currentpage, $route);
  ...
  if ($action == "delete" && $id != '') {
    ...
    $sqlWhere = "joincccs_id =$id";

    if (sqlACTIONS("UPDATE", "joincccs", $arrFields, $arrValues, $sqlWhere)) {
      // sqlACTIONS("UPDATE", "joincccs", $arrFields, $arrValues, $sqlWhere);
      echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
      echo "<script type='text/javascript'>window.location = 'joincccs.php?code=5'; </script>";
    }
  }
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
    $(function() {
      'use strict';
      <?php if ($route == '') : ?>
        $('#joincccsLIST').DataTable({
          // DataTables options here
          'ajax': 'engine/json/JSONjoincccs.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "joincccs_title" //1
            },
            {
              "data": "joincccs_title_hindi" //2
            },
            {
              "data": "joincccs_link" //3
            },
            {
              "data": "joincccs_image" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
            ...
            //     return '<div><h6>In English</h6><p>' + (data || '') + '</p></div><div><h6>In Hindi</h6><p>' + (row.joincccs_title_hindi || '') + '</p></div>';
            //   }
            // },
            {
              "targets": 5,
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="joincccs.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(joincccs_id, status) {
      var SELECTED_ID = joincccs_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__joincccs.php?type=changestatus&joincccs_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__joincccs.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
...
<?php
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add Banner.', 'error');
    }
