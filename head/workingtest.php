...
if ($action == "delete" && $id != '') {
  ...
  if (sqlACTIONS("UPDATE", "workingtest", $arrFields, $arrValues, $sqlWhere)) {
    ...
    echo "<script type='text/javascript'>window.location = 'workingtest.php?code=5'; </script>";
  }
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') { ?>
        $('#workingtestLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONworkingtest.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "workingtest_content" //1
            },
            {
              "data": "workingtest_content_hindi" //2
            },
            {
              "data": "workingtest_image" //3
            },
            {
              "data": "workingtest_link" //4
            },
            {
              "data": "status" //5
            },
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="workingtest.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(workingtest_id, status) {
      var SELECTED_ID = workingtest_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__workingtest.php?type=changestatus&workingtest_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__workingtest.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
