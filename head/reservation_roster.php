...
if ($action == "delete" && $id != '') {
  ...
  $sqlWhere = "reservation_roster_id=$id";

  if (sqlACTIONS("UPDATE", "reservation_roster", $arrFields, $arrValues, $sqlWhere)) {
    echo "<div style='width: 350px; text-align: center; margin: 10% auto 0px; font-family: arial; font-size: 14px; border: 1px solid #ddd; padding: 20px 40px;'>Please wait while we update...</div>";
    echo "<script type='text/javascript'>window.location = 'reservation_roster.php?code=5'; </script>";
  }
}
  ...
  <script>
    $(function() {
      ...
      <?php if ($route == '') {  ?>
        ...
        $('#reservation_rosterLIST').DataTable({
          //responsive: true,
          'ajax': 'engine/json/JSONreservation_roster.php?show=<?php echo $show; ?>&language=en',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "reservation_roster_title" //1
            },
            {
              "data": "filename" //2
            },
          ...
          "columnDefs": [{
            ...
            },
            {
              ...
              "render": function(data, type, row) {
                switch (data) {
                  case '1':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="reservation_rostertatus-' + row.modify + '" id="reservation_rostertatus' + row.modify + '" checked="" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="reservation_rostertatus' + row.modify + '">Yes</label></div>';
                    break;
                  case '0':
                    return '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input" name="reservation_rostertatus--' + row.modify + '" id="reservation_rostertatus' + row.modify + '" onChange="togglestatusITEM(' + row.modify + ', ' + data + ');"> <label class="custom-control-label" for="reservation_rostertatus' + row.modify + '">Yes</label></div>';
                    break;
                }
              }
            }, {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="reservation_roster.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
                //return '';
              }
            }
          ],
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__reservation_roster.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(reservation_roster_id, status) {
      var SELECTED_ID = reservation_roster_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__reservation_roster.php?type=changestatus&reservation_roster_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    <?php if ($route == 'add' || $route == 'edit') :  ?>
      $('#reservation_roster_date').datepicker({
        dateFormat: 'dd/mm/yy',
      ...
      $('#reservation_roster_last_date').datepicker({
        dateFormat: 'dd/mm/yy',
        showOtherMonths: true,
        selectOtherMonths: true,
        changeMonth: true,
      ...
      $('#submit_upload_document_vehicle_btn').click(function() {
        ...
        const newlySelectedDocumentLanguage = $('#reservation_roster_document_language_id').val();
      ...
      function displayUploadedFiles(selectedDocumentType, selectedFiles) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          const selectedDocumentTypeID = $('#reservation_roster_document_type_id').text();
          const selectedDocumentLangiageID = $('#reservation_roster_document_language_id').val();
							...
							<div class="p-2 rounded position-relative alert alert-primary">
								...
								<div class="text-center">
									<div class="vendor-vehicle-image-container">
										...
										<input type="text" name="reservation_roster_document_type_id[${counter_image}]" id="reservation_roster_document_type_id" value="${selectedDocumentTypeID}" hidden="" />
										<input type="text" name="reservation_roster_document_language_id[${counter_image}]" id="reservation_roster_document_language_id" value="${selectedDocumentLangiageID}" hidden="" />
          ...
          $('#reservation_roster_document_language_id').find('option:selected').prop('selected', false);
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add reservation_roster.', 'error');
    }
