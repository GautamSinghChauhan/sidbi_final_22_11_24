...
<body>
  ...
  <script>
     if ($("#psigpolicy_content,#psigpolicy_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#psigpolicy_content, textarea#psigpolicy_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#psigpolicyimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__psigpolicy.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(psigpolicy_ID, status) {
      var SELECTED_ID = psigpolicy_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__psigpolicy.php?type=changestatus&psigpolicy_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#psigpolicyname').parsley();
      var old_psigpolicy_name = document.getElementById("encrypt_psigpolicy_name").value;
      window.ParsleyValidator.addValidator('checkpsigpolicyname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigpolicy_name.php',
            method: "POST",
            data: {
              psigpolicy_name: value,
              old_psigpolicy_name: old_psigpolicy_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#psigpolicycode').parsley();
      var old_psigpolicy_name = document.getElementById("encrypt_psigpolicycode").value;
      window.ParsleyValidator.addValidator('checkpsigpolicycode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_psigpolicy_code.php',
            method: "POST",
            data: {
              psigpolicy_code: value,
              old_psigpolicy_code: old_psigpolicy_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add psigpolicy.', 'error');
    }
