...
$generateINCLUDE = viewGENERATOR($currentpage, $route);
  if ($action == "delete" && $id != '') {
    ...
    if (sqlACTIONS("UPDATE", "js_impact_initiatives", $arrFields, $arrValues, $sqlWhere)) {
      ...
      echo "<script type='text/javascript'>window.location = 'impactinitiatives_content.php?code=5'; </script>";
    }
  }
?>
<!DOCTYPE html>
...
<body>
  ...
  <script>
    $(function() {
      'use strict';
      <?php if ($route == '') : ?>
        $('#impactLIST').DataTable({
          // DataTables options here
          'ajax': 'engine/json/JSONimpact.php',
          "columns": [{
              "data": "count" //0
            },
            {
              "data": "impactinitiatives_title" //1
            },
            {
              "data": "impactinitiatives_hindi_title" //2
            },
            {
              "data": "impactinitiatives_description" //3
            },
            {
              "data": "impactinitiatives_description_hindi" //4
            },
            {
              "data": "impactinitiatives_image" //5impactinitiatives_image
            },
            {
              "data": "impactinitiatives_button1" //6
            },
            {
              "data": "status" //7
            },
          ...
          "columnDefs": [{
            ...
            //     return '<div><h6>In English</h6><p>' + (data || '') + '</p></div><div><h6>In Hindi</h6><p>' + (row.home_title_hindi || '') + '</p></div>';
            //   }
            // },
            {
              "targets": 7,
            ...
            },
            {
              ...
              "render": function(data, type, full, meta) {
                return '<a title="Click to edit" href="impactinitiatives_content.php?route=edit&id=' + data + '" class="btn btn-light btn-icon"><i class="fa fa-edit"></i></a> <a href="javascript:void(0);" onClick ="deleteITEM(' + data + ');" title="Click to delete" class="btn btn-dark btn-icon"><i class="fa fa-trash"></i></a>';
              }
            }
          ],
    ...
    function togglestatusITEM(impact_id, status) {
      var SELECTED_ID = impact_id;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__impactinitiatives_content.php?type=changestatus&impact_id=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
    ...
    function deleteITEM(deleting_id) {
      ...
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__impactinitiatives_content.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
        $('#pleasewait-loader').hide();
        $('#deleteDATA').modal({
          show: true
        });
