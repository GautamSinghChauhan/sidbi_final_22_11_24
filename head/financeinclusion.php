...
<body>
  ...
  <script>
     if ($("#financeinclusion_content,#financeinclusion_content_hindi").length > 0) {
        tinymce.init({
            selector: "textarea#financeinclusion_content, textarea#financeinclusion_content_hindi",
            theme: "modern",
            menubar: false,
            min_height: 300,
    ...
    $(function() {
      'use strict'
        ...
        $("#financeinclusionimage").change(function() {
          uploadImage(this);
        });
    ...
    function deleteITEM(deleting_id) {
      ...
      $('.receiving-delete-data').load('view/x__financeinclusion.php?type=delete&delete_id=' + SELECTED_ID + '', function() {
    ...
    function togglestatusITEM(financeinclusion_ID, status) {
      var SELECTED_ID = financeinclusion_ID;
      var SELECTED_STATUS = status;
      //alert(SELECTED_ID);
      $('#pleasewait-loader').show();
      $('.receiving-delete-data').load('view/x__financeinclusion.php?type=changestatus&financeinclusion_ID=' + SELECTED_ID + '&oldstatus=' + SELECTED_STATUS + '', function() {
        $('#pleasewait-loader').hide();
        location.reload();
      });
    }
    ...
    $(document).ready(function() {
      $('#financeinclusionname').parsley();
      var old_financeinclusion_name = document.getElementById("encrypt_financeinclusion_name").value;
      window.ParsleyValidator.addValidator('checkfinanceinclusionname', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_financeinclusion_name.php',
            method: "POST",
            data: {
              financeinclusion_name: value,
              old_financeinclusion_name: old_financeinclusion_name
            },
            dataType: "json",
    ...
    $(document).ready(function() {
      $('#financeinclusioncode').parsley();
      var old_financeinclusion_name = document.getElementById("encrypt_financeinclusioncode").value;
      window.ParsleyValidator.addValidator('checkfinanceinclusioncode', {
        validateString: function(value) {
          return $.ajax({
            url: 'engine/ajax/ajax_financeinclusion_code.php',
            method: "POST",
            data: {
              financeinclusion_code: value,
              old_financeinclusion_code: old_financeinclusion_name
            },
            dataType: "json",
            success: function(data) {
    ...
    if ($code == '0') {
      $displayMSG_globalclass->displayMSG($code, "Error", 'Unable to Add financeinclusion.', 'error');
    }
