...
<body class="main-wrapper-en">
    ...
    <div class="section-apply">
        <div class="row g-0">
            <?php
            $list_transaction = sqlQUERY_LABEL("SELECT `machinerybanner_ID`, `machinerybanner_title`, `machinerybanner_title_hindi`, `machinerybanner_image`, `status` FROM `machinerybanner` where deleted = '0' and `status` = '1'") or die("Unable to get records:" . sqlERROR_LABEL());
            ...
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-5 col-xxl-5 p-0 ">
                <img src="<?= SEOURL; ?>assets/front/home/<?= $machinerybanner_attachement ?>" title="#" alt="#"
                    class="section-image">
            </div>
                ...
                <div>
                    <div>
                        <h5 class="section-head mx-5 mx-xl-0 my-3">  <?php  if($get_configured_lang == 'HI'): 
                                                 echo   $machinerybanner_title_hindi;
                                                else:
                                                 echo   $machinerybanner_title;
                                                endif; ?></h5>
                    </div>
    ...
    <?php
    $select_machineryproduct = sqlQUERY_LABEL("SELECT `machineryproduct_id`, `machineryproduct_tab`, `machineryproduct_tab_hindi` FROM `machineryproduct` WHERE `status` = '1' and `deleted` = '0' ORDER BY `machineryproduct_id` ASC") or die("Unable to get records:" . sqlERROR_LABEL());
    $total_machineryproduct_count = sqlNUMOFROW_LABEL($select_machineryproduct);
    if ($total_machineryproduct_count > 0) :
    ?>
    <div class="mechinery-product">
        <div data-aos="fade-up" data-aos-duration="2000">
            <h5 class="machinery-heading py-4 py-md-5 mx-4 mb-0" ><?php  if($get_configured_lang == 'HI'): 
                                                 echo   'मशीनरी उत्पाद';
                                                else:
                                                 echo   'Machinery Products';
                                                endif; ?>
            </h5>
            <div class="d-flex justify-content-center mb-4 mb-md-5">
                <ul class="nav nav-pills mb-3 machinery-tap" id="pills-tab" role="tablist">
                    <?php
                        $machineryproduct_counter = 0;
                        while ($row_machineryproduct = sqlFETCHARRAY_LABEL($select_machineryproduct)) :
                            $machineryproduct_counter++;
                            $machineryproduct_id = $row_machineryproduct["machineryproduct_id"];
                            $machineryproduct_tab = $row_machineryproduct["machineryproduct_tab"];
                            $machineryproduct_tab_hindi = $row_machineryproduct["machineryproduct_tab_hindi"];

                            if ($machineryproduct_counter == 1) :
                                $get_active_machinery_product = $machineryproduct_id;
                            endif;

                        ?>
                    <li class="nav-item col-6" role="presentation">
                        <button class="nav-link machinery-button"
                            onclick="fetch_MACHINERY_DETAILS('<?= $machineryproduct_id; ?>')" type="button"
                            id="machineryproduct_btn_<?= $machineryproduct_id; ?>" role="tab"
                            aria-selected="true">  <?php  if($get_configured_lang == 'HI'): 
                                                 echo   $machineryproduct_tab_hindi;
                                                else:
                                                 echo   $machineryproduct_tab;
                                                endif; ?></button>
                    </li>
                    <?php endwhile; ?>
                </ul>
            ...
            <div class="tab-content container">
                <div class="tab-pane fade show active" id="machineryproduct_<?= $machineryproduct_id; ?>"
                    role="tabpanel" aria-labelledby="pills-home-tab">
                    <span id="show_ajax_machinery_product_response"></span>
                </div>
            </div>
    ...
    <div class="container testimonials-container" data-aos="fade-up" data-aos-duration="2000">
        <div class="owl-carousel owl-theme">
            <?php
            $list_transaction = sqlQUERY_LABEL("SELECT `machinerytest_id`, `machinerytest_title`, `machinerytest_title_hindi`, `machinerytest_content`, `machinerytest_content_hindi`, `machinerytest_image`, `machinerytest_link`,  `status` FROM `machinerytest` where deleted = '0' and `status` = '1'") or die("Unable to get records:" . sqlERROR_LABEL());
            while ($row = sqlFETCHARRAY_LABEL($list_transaction)) :
            ...
            <div class="item">
                <div class="card success-card">
                    <div class="card-head">
                        <img src="assets/front/home/<?= $machinerytest_image ?>">
                        <p class="success-p">  <?php  if($get_configured_lang == 'HI'): 
                                                 echo   $machinerytest_content_hindi;
                                                else:
                                                 echo   $machinerytest_content;
                                                endif; ?>
                        </p>
                    </div>
                    ...
                    <div class="card-footer success-card-footer">
                        <div class="d-flex justify-content-between">
                            <h5 class="success-story-foot"><a href="<?= $machinerytest_link ?>" target="_blank"
                                  ><?php  if($get_configured_lang == 'HI'): 
    ...
    <div class="">
        <div class="swapping-card">
            ...
            <div class="row g-0 gap-5 justify-content-center swapping-card-row">
                <?php
                $list_transaction = sqlQUERY_LABEL("SELECT `machineryother_id`, `machineryother_title`, `machineryother_title_hindi`, `machineryother_name`, `machineryother_name_hindi`, `machineryother_image`, `machineryother_link`,  `status` FROM `machineryother` where deleted = '0' and `status` = '1'") or die("Unable to get records:" . sqlERROR_LABEL());
                ...
                <div class="col-xl-3 col-lg-4 col-md-6 col-12 p-0 ">
                    <div class="man-content mx-5 mx-md-0" data-aos="fade-up" data-aos-duration="2000">
                        <div>
                            <a href="<?= $machineryother_link ?>">
                                <h2 class="offering-title m-md-0 m-sm-0">  <?php  if($get_configured_lang == 'HI'): 
                                                 echo   $machineryother_name_hindi;
                                                else:
                                                 echo   $machineryother_name;
                                                endif; ?></h2>
                                <img src="<?= SEOURL; ?>assets/front/home/<?= $machineryother_image ?>"
                                    class="img-fluid">
                            </a>
                        </div>
                    </div>
    ...
    $(document).ready(function() {
        $('.carousel').carousel();
        fetch_MACHINERY_DETAILS('<?= $get_active_machinery_product; ?>');
    });
    </script>
    ...
    function fetch_MACHINERY_DETAILS(MACHINERY_PRDT_ID) {
        $.ajax({
            url: 'head/engine/ajax/ajax_fetch_machineryproduct_details.php',
            type: 'POST',
            data: {
                MACHINERY_PRDT_ID: MACHINERY_PRDT_ID
            },
            success: function(data) {
                $('#show_ajax_machinery_product_response').html(data);
                $('.nav-link').removeClass('active');
                $('#machineryproduct_btn_' + MACHINERY_PRDT_ID).addClass('active');
            }
        });
    }
    </script>
