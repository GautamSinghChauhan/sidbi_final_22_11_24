    ...
    var g = {
        ...
        sendColPrefs: function () {
            if ($(g.t).is('.ajax')) {   // only send preferences if ajax class
                ...
                $.post('index?route=/sql/set-column-preferences', postParams, function (data) {
        ...
        showEditArea: function () {
            if (!g.isCellEditActive) {   // make sure the edit area has not been shown
                ...
                if ($td.is('.relation')) {
                    ...
                    g.lastXHR = $.post('index?route=/sql/get-relational-values', postParams, function (data) {
                        g.lastXHR = null;
                    ...
                    g.lastXHR = $.post('index?route=/sql/get-enum-values', postParams, function (data) {
                        g.lastXHR = null;
                    ...
                    g.lastXHR = $.post('index?route=/sql/get-set-values', postParams, function (data) {
                        g.lastXHR = null;
                        if (typeof data === 'object' && data.success === false) {
                            Functions.ajaxShowMessage(data.error, undefined, 'error');
                            return;
                    ...
                    if ($td.is('.to_be_saved')) {   // cell has been edited
                        ...
                        g.lastXHR = $.post('index?route=/sql', {
                            'server' : g.server,
                            'db' : g.db,
                            'ajax_request' : true,
                            'sql_query' : sqlQuery,
        ...
        postEditedCell: function (options) {
            ...
            var postParams = { 'ajax_request' : true,
                ...
                'goto' : encodeURIComponent('index?route=/sql'),
                'submit_type' : 'save'
            };
