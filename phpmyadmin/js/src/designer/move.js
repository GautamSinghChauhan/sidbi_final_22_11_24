...
DesignerMove.addOtherDbTables = function () {
    ...
    $('#designerModalGoButton').on('click', function () {
        ...
        $.post('index?route=/database/designer', {
            'ajax_request' : true,
            'dialog' : 'add_table',
            'db' : db,
            'table' : table,
...
DesignerMove.save2 = function (callback) {
    if (designerTablesEnabled) {
        ...
        $.post('index?route=/database/designer', poststr, function (data) {
...
DesignerMove.save3 = function (callback) {
    if (selectedPage !== -1) {
        ...
        var $form = $('<form action="index?route=/database/designer" method="post" name="save_page" id="save_page" class="ajax"></form>')
...
DesignerMove.loadPage = function (page) {
    if (designerTablesEnabled) {
        ...
        $('<a href="index?route=/database/designer&server=' + server + argsep + 'db=' + encodeURIComponent(db) + paramPage + '"></a>')
...
DesignerMove.newRelation = function () {
    ...
    var $msgbox = Functions.ajaxShowMessage(Messages.strProcessingRequest);
    $.post('index?route=/database/designer', linkRelation, function (data) {
        if (data.success === false) {
            Functions.ajaxShowMessage(data.error, false);
        } else {
            Functions.ajaxRemoveMessage($msgbox);
...
DesignerMove.startTableNew = function () {
    ...
    CommonActions.refreshMain('index?route=/table/create');
};

DesignerMove.startTabUpd = function (db, table) {
    CommonParams.set('db', db);
    CommonParams.set('table', table);
    CommonActions.refreshMain('index?route=/table/structure');
};
