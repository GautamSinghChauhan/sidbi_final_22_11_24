...
AJAX.registerOnload('functions.js', function () {
    ...
    $(document).on('click', 'input#sql_query_edit_save', function () {
        ...
        var $fakeForm = $('<form>', { action: 'index?route=/import', method: 'post' })
            ...
            if (Functions.checkReservedWordColumns($form)) {
                ...
                $.post($form.attr('action'), $form.serialize() + CommonParams.get('arg_separator') + 'do_save_data=1', function (data) {
                    if (typeof data !== 'undefined' && data.success === true) {
                        ...
                        var tableStructureUrl = 'index?route=/table/structure' + argsep + 'server=' + data.params.server +
                            argsep + 'db=' + data.params.db + argsep + 'token=' + data.params.token +
                            argsep + 'goto=' + encodeURIComponent('index?route=/database/structure') + argsep + 'table=' + data.params.table + '';
