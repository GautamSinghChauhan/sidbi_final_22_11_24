...
AJAX.registerOnload('sql.js', function () {
        ...
        $.post($form.attr('action'), $form.serialize() + argsep + 'ajax_page_request=true', function (data) {
            if (typeof data !== 'undefined' && data.success === true) {
                ...
                if (typeof data.ajax_reload !== 'undefined') {
                    ...
                    if (data.db) {
                        if (data.table) {
                            url = 'index?route=/table/sql';
                        } else {
                            url = 'index?route=/database/sql';
                        }
                    } else {
                        url = 'index?route=/server/sql';
                    }
    ...
    $('body').on('click', '#simulate_dml', function () {
        ...
        $.ajax({
            type: 'POST',
            url: 'index?route=/import/simulate-dml',
            data: {
    ...
    $('body').on('click', 'form[name="resultsForm"].ajax button[name="submit_mult"], form[name="resultsForm"].ajax input[name="submit_mult"]', function (e) {
        ...
        if (action === 'edit') {
            ...
            url = 'index?route=/table/change/rows';
        } else if (action === 'copy') {
            submitData = submitData + argsep + 'default_action=insert';
            url = 'index?route=/table/change/rows';
        } else if (action === 'export') {
            url = 'index?route=/table/export/rows';
        } else if (action === 'delete') {
            url = 'index?route=/table/delete/confirm';
        } else {
            return;
        }
