...
Functions.insertQuery = function (queryType) {
  ...
  if (queryType === 'clear') {
    ...
    if (codeMirrorEditor) {
      ...
      $.ajax({
        type: 'POST',
        url: 'index?route=/database/sql/format',
        data: params,
        success: function (data) {
          if (data.success) {
...
Functions.loadForeignKeyCheckbox = function () {
  ...
  $.get('index?route=/sql/get-default-fk-check-value', params, function (data) {
...
Functions.codeMirrorAutoCompleteOnInputRead = function (instance) {
  if (!sqlAutoCompleteInProgress && (!instance.options.hintOptions.tables || !sqlAutoComplete)) {
    if (!sqlAutoComplete) {
      ...
      $.ajax({
        type: 'POST',
        url: 'index?route=/database/sql/autocomplete',
        data: params,
        success: function (data) {
          if (data.success) {
...
Functions.checkReservedWordColumns = function ($form) {
  ...
  $.ajax({
    type: 'POST',
    url: 'index?route=/table/structure/reserved-word-check',
    data: $form.serialize(),
    success: function (data) {
      ...
      if (Functions.checkReservedWordColumns($form)) {
        ...
        $.post($form.attr('action'), $form.serialize() + CommonParams.get('arg_separator') + 'do_save_data=1', function (data) {
          if (typeof data !== 'undefined' && data.success === true) {
            ...
            var tableStructureUrl = 'index?route=/table/structure' + argsep + 'server=' + data.params.server + argsep + 'db=' + data.params.db + argsep + 'token=' + data.params.token + argsep + 'goto=' + encodeURIComponent('index?route=/database/structure') + argsep + 'table=' + data.params.table + '';
...
/**
 * Attach Ajax event handlers for 'Change Password' on index
 */
AJAX.registerOnload('functions.js', function () {
...
AJAX.registerOnload('functions.js', function () {
  ...
  $(document).on('click', 'a.central_columns_dialog', function () {
    var href = 'index?route=/database/central-columns';
    var db = CommonParams.get('db');
    var table = CommonParams.get('table');
...
Functions.indexEditorDialog = function (url, title, callbackSuccess, callbackFailure) {
  Functions.indexDialogModal('index?route=/table/indexes', url, title, callbackSuccess, callbackFailure);
};
Functions.indexRenameDialog = function (url, title, callbackSuccess, callbackFailure) {
  Functions.indexDialogModal('index?route=/table/indexes/rename', url, title, callbackSuccess, callbackFailure);
};
  ...
  $.post($this.attr('href'), params, function (data) {
    if (typeof data !== 'undefined' && data.success === true) {
      ...
      $('#createViewModalGoButton').on('click', function () {
        ...
        $.post('index?route=/view/create', $('#createViewModal').find('form').serialize(), function (data) {
          Functions.ajaxRemoveMessage($msg);
          if (typeof data !== 'undefined' && data.success === true) {
...
AJAX.registerOnload('functions.js', function () {
   ...
   * method is selected
   * Used in /user-password (Change Password link on index)
   */
