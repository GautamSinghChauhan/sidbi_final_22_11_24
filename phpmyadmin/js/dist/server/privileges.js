...
const EditUserGroup = {
  ...
  handleEvent: function (event) {
    ...
    $.get('index?route=/server/user-groups/edit-form', {
      'username': username,
      ...
      saveButton.addEventListener('click', () => {
        ...
        $.post('index?route=/server/privileges', form.serialize() + CommonParams.get('arg_separator') + 'ajax_request=1', data => {
          if (typeof data === 'undefined' || data.success !== true) {
            Functions.ajaxShowMessage(data.error, false, 'error');
            return;
          }
...
const AccountLocking = {
  handleEvent: function () {
    ...
    const url = isLocked ? 'index?route=/server/privileges/account-unlock' : 'index?route=/server/privileges/account-lock';
    const params = {
