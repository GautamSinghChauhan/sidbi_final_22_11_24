...
function loadGISEditor(value, field, type, inputName) {
  ...
  $.post('index?route=/gis-data-editor', {
    'field': field,
    'value': value,
    'type': type,
...
function insertDataAndClose() {
  ...
  $.post('index?route=/gis-data-editor', $form.serialize() + argsep + 'generate=true' + argsep + 'ajax_request=true', function (data) {
    if (typeof data !== 'undefined' && data.success === true) {
...
AJAX.registerOnload('gis_data_editor.js', function () {
  ...
  $(document).on('change', '#gis_editor select.gis_type', function () {
    ...
    var $form = $('form#gis_data_editor_form');
    var argsep = CommonParams.get('arg_separator');
    $.post('index?route=/gis-data-editor', $form.serialize() + argsep + 'get_gis_editor=true' + argsep + 'ajax_request=true', function (data) {
      if (typeof data !== 'undefined' && data.success === true) {
        $gisEditor.html(data.gis_editor);
        initGISEditorVisualization();
        prepareJSVersion();
