...
AJAX.registerOnload('database/structure.js', function () {
  ...
  $('select[name=submit_mult]').on('change', function (event) {
    var url = 'index?route=/database/structure';
    var action = $(this).val();
    if (action === 'make_consistent_with_central_list') {
      ...
      $('#makeConsistentWithCentralListModal').modal('show').on('shown.bs.modal', function () {
        $('#makeConsistentWithCentralListContinue').on('click', function () {
          ...
          AJAX.source = $form;
          $.post('index?route=/database/structure/central-columns/make-consistent', data, AJAX.responseHandler);
    ...
    if (action === 'copy_tbl' || action === 'add_prefix_tbl' || action === 'replace_prefix_tbl' || action === 'copy_tbl_change_prefix') {
      ...
      var modalTitle = '';
      if (action === 'copy_tbl') {
        url = 'index?route=/database/structure/copy-form';
        modalTitle = Messages.strCopyTablesTo;
      } else if (action === 'add_prefix_tbl') {
        url = 'index?route=/database/structure/add-prefix';
        modalTitle = Messages.strAddPrefix;
      } else if (action === 'replace_prefix_tbl') {
        url = 'index?route=/database/structure/change-prefix-form';
    ...
    if (action === 'analyze_tbl') {
      url = 'index?route=/table/maintenance/analyze';
    } else if (action === 'sync_unique_columns_central_list') {
      url = 'index?route=/database/structure/central-columns/add';
    } else if (action === 'delete_unique_columns_central_list') {
      url = 'index?route=/database/structure/central-columns/remove';
    } else if (action === 'check_tbl') {
      url = 'index?route=/table/maintenance/check';
    } else if (action === 'checksum_tbl') {
      url = 'index?route=/table/maintenance/checksum';
    } else if (action === 'drop_tbl') {
      url = 'index?route=/database/structure/drop-form';
    } else if (action === 'empty_tbl') {
      url = 'index?route=/database/structure/empty-form';
    } else if (action === 'export') {
      url = 'index?route=/export/tables';
    } else if (action === 'optimize_tbl') {
      url = 'index?route=/table/maintenance/optimize';
    } else if (action === 'repair_tbl') {
      url = 'index?route=/table/maintenance/repair';
    } else if (action === 'show_create') {
      url = 'index?route=/database/structure/show-create';
    } else {
